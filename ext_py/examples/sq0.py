...
def main():
    ...
    doubled = ext_py.DoubleChainComplex(res)
    doubled.compute_through_bidegree(
        sseq_py.Bidegree.s_t(res.next_homological_degree() - 1, 0)
    )
    ...
    hom.extend_step_raw(
        sseq_py.Bidegree.zero(),
        [fp_py.FpVector.from_slice(res.prime(), [1])],
    )
    hom.extend_all()

    for b in res.iter_nonzero_stem():
        doubled_b = sseq_py.Bidegree.s_t(b.s(), 2 * b.t())
