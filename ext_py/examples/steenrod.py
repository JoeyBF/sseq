...
import ext_py
from ext_py import fp_py, sseq_py
from ext_py.algebra_py import FreeModuleHomomorphism
...
# `ext_py.TensorChainComplex` providing `.new`, `.module`, `.differential`,
...
def main():
    ...
    yoneda = ext_py.yoneda_representative_element(resolution, b, class_)
    ...
    # Rust-side machinery).
    square = ext_py.TensorChainComplex.new(yoneda, yoneda)
    doubled_b = b + b
    ...
    for i in range(b.s() + 1):
        shift_s = sseq_py.Bidegree.s_t(i, 0)
        ...
        for s in range((doubled_b - shift_s).s() + 1):
            if i == 0 and s == 0:
                ...
                m.add_generators_from_matrix_rows(
                    0, fp_py.Matrix.from_vec(p, [[1]]).slice_mut(0, 1, 0, 1)
                )
            ...
            for t in range(doubled_b.t() + 1):
                ...
                output_matrix = fp_py.Matrix(p, num_gens, fx_dim)
                result = fp_py.FpVector(p, fdx_dim)
                for j in range(num_gens):
                    if prev_delta is not None:
                        ...
                        square.swap(
                            result, prevd, bb + shift_s - sseq_py.Bidegree.s_t(1, 0)
                        )
        ...
        basis_string = sseq_py.BidegreeElement(
            b, fp_py.FpVector.from_slice(p, class_)
        ).to_basis_string()
        result = ", ".join(
