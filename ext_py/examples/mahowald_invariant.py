...
def resolve_s_2(s_2_path, k_max):
    ...
    s_2_resolution = ext_py.construct_standard("S_2", s_2_path)
    ...
    s_2_resolution.compute_through_stem(
        sseq_py.Bidegree.n_s(2 * k_max - 2, k_max // 2 + 1)
    )
...
class MahowaldInvariant:
    ...
    def __str__(self):
        ...
        def f2_vec_to_sum(v):
            elt = sseq_py.BidegreeElement(
                sseq_py.Bidegree.s_t(self.g.s(), output_t), v
            )
...
class PKData:
    def __init__(self, k, p_k_prefix, s_2_resolution):
        ...
        self.resolution = ext_py.construct_standard(
            (p_k_config, algebra_py.AlgebraType.MILNOR), p_k_path
        )
        # RP_-k_inf won't detect Mahowald invariants of classes in the k-stem and
        # beyond or of filtration higher than k/2+1.
        self.resolution.compute_through_stem(sseq_py.Bidegree.n_s(k - 2, k // 2 + 1))

        self.bottom_cell = ext_py.ResolutionHomomorphism.from_class(
            "bottom_cell",
            self.resolution,
            s_2_resolution,
            sseq_py.Bidegree.s_t(0, -k),
            [1],
        )
        self.bottom_cell.extend_all()

        self.minus_one_cell = ext_py.ResolutionHomomorphism.from_class(
            "minus_one_cell",
            self.resolution,
            s_2_resolution,
            sseq_py.Bidegree.s_t(0, -1),
            [1],
        )
    ...
    def mahowald_invariants_for_bidegree(self, b):
        b_p_k = b - sseq_py.Bidegree.s_t(0, 1)
        if self.resolution.has_computed_bidegree(b_p_k):
            b_bottom = b_p_k + sseq_py.Bidegree.s_t(0, self.k)
            ...
            if bottom_s_2_gens > 0 and minus_one_s_2_gens > 0 and p_k_gens > 0:
                ...
                padded_columns, matrix = fp_py.Matrix.augmented_from_vec(TWO, matrix)
                ...
                if rank > 0:
                    ...
                    for i in range(minus_one_s_2_gens):
                        image = fp_py.FpVector.new(TWO, p_k_gens)
                        ...
                        if not image.is_zero() and image_subspace.contains(
                            ...
                            invariant = fp_py.FpVector.new(TWO, bottom_s_2_gens)
                            quasi_inverse.apply(
