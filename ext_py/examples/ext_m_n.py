...
class HomCochainComplex:
    ...
    def compute_through_stem(self, max):
        for s in range(len(self.modules), max.s() + 1):
            self.modules.append(
                algebra_py.HomModule(self.source.module(s), self.target)
            )
        for s in range(len(self.differentials), max.s()):
            self.differentials.append(
                algebra_py.HomPullback(
                    self.modules[s],
                    self.modules[s + 1],
    ...
    def homology_dimension(self, b):
        ...
        return fp_py.Subquotient.from_parts(
...
def main():
    ...
    module_spec = query.raw("Module N", ext_py.parse_module_name)
    module = algebra_py.steenrod_module_from_json(res.algebra(), module_spec)
    ...
    res.compute_through_stem(max + sseq_py.Bidegree.n_s(module.max_degree(), 1))
    ...
    for s in range(max.s(), -1, -1):
        for n in range(hom_cc.min_degree(), max.n() + 1):
            ...
            result += ext_py.unicode_num(hom_cc.homology_dimension(b))
            result += " "
        result += "\n"
