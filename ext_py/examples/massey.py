...
def main():
    ...
    shift = a + b - sseq_py.Bidegree.s_t(1, 0)

    if not is_unit:
        unit.compute_through_stem(shift)

    if not resolution.has_computed_bidegree(
        shift + sseq_py.Bidegree.s_t(0, resolution.min_degree())
    ):
        return

    b_hom = ext_py.ResolutionHomomorphism.from_class("", unit, unit, b, b_class)
    ...
    for c in resolution.iter_nonzero_stem():
        ...
        product = fp_py.AugmentedMatrix2(p, num_gens, [product_num_gens, num_gens])
        product.segment(1, 1).add_identity()

        matrix = fp_py.Matrix(p, num_gens, 1)
        for idx in range(num_gens):
            hom = ext_py.ResolutionHomomorphism("", resolution, unit, c)
            ...
            homotopy = ext_py.ChainHomotopy(hom, b_hom)
            homotopy.extend(tot)
            ...
            for k, v in enumerate(b_class):
                if v != 0:
                    g = sseq_py.BidegreeGenerator(b, k)
        ...
        for row in kernel.iter():
            c_element = sseq_py.BidegreeElement(c, row.to_owned())
            entries = []
            for i in range(target_num_gens):
                entry = 0
