...
def main():
    ...
    hom = ext_py.ResolutionHomomorphism(name, resolution, unit, shift)

    matrix = fp_py.Matrix(p, hom.source.number_of_gens_in_bidegree(shift), 1)
    ...
    unit_lift = res_lift
    if not is_unit:
        unit_lift = ext_py.SecondaryResolution(unit)
        unit_lift.extend_all()

    hom_lift = ext_py.SecondaryResolutionHomomorphism(res_lift, unit_lift, hom)
    ...
    for b in unit.iter_nonzero_stem():
        ...
        if not resolution.has_computed_bidegree(b + shift + ext_py.LAMBDA_BIDEGREE):
            continue
        if not resolution.has_computed_bidegree(b + shift - sseq_py.Bidegree.s_t(1, 0)):
            continue
        ...
        lambda_num_gens = resolution.number_of_gens_in_bidegree(
            b + shift + ext_py.LAMBDA_BIDEGREE
        )
        ...
        outputs = [
            fp_py.FpVector(p, target_num_gens + lambda_num_gens)
        ...
        for g, output in zip(page_data.subspace_gens(), outputs):
            basis_string = sseq_py.BidegreeElement(b, g.to_owned()).to_basis_string()
            print(
                f"{name} [{basis_string}] = "
