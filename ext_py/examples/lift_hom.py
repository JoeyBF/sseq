...
def main():
    ...
    def parse_target(s):
        ...
        target = ext_py.construct(s, save_dir)
        target.set_name(s)
        return target
    ...
    hom = ext_py.ResolutionHomomorphism(name, source, target, shift)

    print("\nInput Ext class to lift:", file=sys.stderr)
    for output_t in range(0, target_module.max_degree() + 1):
        output = sseq_py.Bidegree.s_t(0, output_t)
        input = output + shift
        matrix = fp_py.Matrix(
            p,
        ...
        if matrix.rows() == 0 or matrix.columns() == 0:
            ...
            for idx in range(matrix.rows()):
                ...
                g = sseq_py.BidegreeGenerator(input, idx)
    ...
    for b2 in hom.target.iter_stem():
        ...
        for i, r in enumerate(matrix):
            g = sseq_py.BidegreeGenerator(b2, i)
