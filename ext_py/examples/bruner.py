...
from ext_py import algebra_py, construct, fp_py, sseq_py
from ext_py.algebra_py import MilnorAlgebra, MilnorBasisElement, SteenrodAlgebra
# NOTE: depends on FreeModule, FreeModuleHomomorphism (algebra_py, API_PROPOSAL §5.3/§5.4),
# FiniteChainComplex (ext_py top level, API_PROPOSAL §7.1), ResolutionHomomorphism
# (ext_py top level, API_PROPOSAL §7.3), construct (ext_py, API_PROPOSAL §7.7).
from ext_py import FiniteChainComplex, ResolutionHomomorphism
# NOTE: depends on Matrix.from_vec (fp_py, API_PROPOSAL §4.4) and the TWO constant
# (fp_py, API_PROPOSAL §4.1).
from ext_py.fp_py import FpVector, Matrix
...
def create_chain_complex(num_s):
    ...
    algebra = SteenrodAlgebra.milnor(int(TWO), False)

    modules = []
    differentials = []
    for _ in range(num_s):
        modules.append(algebra_py.FreeModule(algebra, "", 0))
    for s in range(1, num_s):
        differentials.append(
            algebra_py.FreeModuleHomomorphism(modules[s], modules[s - 1], 0)
        )
...
def read_bruner_resolution(data_dir, max_n):
    ...
    algebra = MilnorAlgebra(int(TWO), False)
    ...
    for s in range(1, num_s):
        ...
        with open(os.path.join(data_dir, f"hDiff.{s}")) as f:
            ...
            while True:
                element = get_element(algebra, cc.module(s - 1), f)
                if element is None:
                    break
                t, g = element
                if t == cur_degree:
...
def main():
    ...
    max = sseq_py.Bidegree.n_s(max_n, max_s)
    ...
    hom = ResolutionHomomorphism("", cc, resolution, sseq_py.Bidegree.n_s(0, 0))

    # We have to explicitly tell it what to do at (0, 0)
    hom.extend_step(sseq_py.Bidegree.n_s(0, 0), Matrix.from_vec(TWO, [[1]]))
    hom.extend_all()
