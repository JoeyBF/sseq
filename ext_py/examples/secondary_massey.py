...
def get_hom(name, source, target):
    ...
    source.underlying().compute_through_stem(shift + ext_py.LAMBDA_BIDEGREE)

    hom = ext_py.ResolutionHomomorphism(
        ext_name, source.underlying(), target.underlying(), shift
    )

    num_gens = source.underlying().number_of_gens_in_bidegree(shift)
    num_lambda_gens = hom.source.number_of_gens_in_bidegree(shift + ext_py.LAMBDA_BIDEGREE)

    class_ = fp_py.FpVector(p, num_gens + num_lambda_gens)

    matrix = fp_py.Matrix(p, num_gens, 1)
    ...
    hom_lift = ext_py.SecondaryResolutionHomomorphism(source, target, hom)
    ...
    if num_lambda_gens > 0:
        ...
        if lambda_name == "":
            ...
            lambda_part = ext_py.ResolutionHomomorphism.from_class(
                lambda_name,
                hom_lift.source(),
                hom_lift.target(),
                shift + ext_py.LAMBDA_BIDEGREE,
                v,
            )
...
def main():
    ...
    is_unit, unit = ext_py.get_unit(resolution)

    p = resolution.prime()

    res_lift = ext_py.SecondaryResolution(resolution)
    unit_lift = res_lift if is_unit else ext_py.SecondaryResolution(unit)
    ...
    chain_homotopy = ext_py.ChainHomotopy(a.underlying(), b.underlying())
    ...
    homotopy.add_generators_from_rows(
        htpy_source.t(),
        [fp_py.FpVector.from_slice(p, [x]) for x in v],
    )

    chain_homotopy.extend_all()

    ch_lift = ext_py.SecondaryChainHomotopy(
    ...
    for c in unit.iter_stem():
        if not resolution.has_computed_bidegree(
            c + shift - sseq_py.Bidegree.s_t(2, 0)
        ) or not resolution.has_computed_bidegree(c + shift + sseq_py.Bidegree.s_t(0, 1)):
            continue

        # Now read off the products
        source = c + shift - sseq_py.Bidegree.s_t(1, 0)

        source_num_gens = resolution.number_of_gens_in_bidegree(source)
        source_lambda_num_gens = resolution.number_of_gens_in_bidegree(
            source + ext_py.LAMBDA_BIDEGREE
        )
        ...
        target_lambda_num_gens = unit.number_of_gens_in_bidegree(c + ext_py.LAMBDA_BIDEGREE)
        ...
        prod_lambda_num_gens = unit.number_of_gens_in_bidegree(
            c + b_shift + ext_py.LAMBDA_BIDEGREE
        )
        prod_all_gens = prod_num_gens + prod_lambda_num_gens

        target_page_data = get_page_data(unit_sseq, c)
        target_lambda_page_data = get_page_data(unit_sseq, c + ext_py.LAMBDA_BIDEGREE)
        product_lambda_page_data = get_page_data(
            unit_sseq, c + b_shift + ext_py.LAMBDA_BIDEGREE
        )
        ...
        for g in e3_kernel.iter():
            ...
            if has_ext:
                basis_string = sseq_py.BidegreeElement(
                    c, ext_part.to_owned()
                ).to_basis_string()
            ...
            for i, val in enumerate(scratch0):
                ...
                for gen_idx in range(source_lambda_num_gens):
                    mm = a.underlying().get_map((source + ext_py.LAMBDA_BIDEGREE).s())
                    dx = mm.output((source + ext_py.LAMBDA_BIDEGREE).t(), gen_idx)
