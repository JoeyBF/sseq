...
def make_c2_fdmodule():
    ...
    """
    m = algebra_py.FDModuleBuilder(milnor(2), "C2", [1, 1])
...
def test_fdmodule_act_on_basis_known_value():
    ...
    res = fp_py.FpVector(2, m.dimension(1))
...
def test_fdmodule_act_with_element_input():
    ...
    inp = fp_py.FpVector(2, m.dimension(0))
    inp[0] = 1
    res = fp_py.FpVector(2, m.dimension(1))
...
def test_fdmodule_build():
    m = make_c2_fdmodule()
    sm = m.build()
    assert isinstance(sm, algebra_py.SteenrodModule)
...
def test_steenrod_module_from_json_c2():
    sm = algebra_py.steenrod_module_from_json(milnor(2), C2_JSON)
    assert isinstance(sm, algebra_py.SteenrodModule)
    assert sm.prime() == 2
...
def test_steenrod_module_from_json_action_known_value():
    sm = algebra_py.steenrod_module_from_json(milnor(2), C2_JSON)
    # Sq1 . x0 = x1.
    res = fp_py.FpVector(2, sm.dimension(1))
...
def test_steenrod_module_from_json_bad_spec_raises():
    with pytest.raises(ValueError):
        algebra_py.steenrod_module_from_json(milnor(2), {"p": 2})  # missing type
    with pytest.raises(ValueError):
        algebra_py.steenrod_module_from_json(milnor(2), {"p": 2, "type": "bogus"})
...
def make_free(gen_degrees=(0,)):
    ...
    b = algebra_py.FPModuleBuilder(milnor(2), "F", 0)
...
def test_freemodule_index_to_op_gen():
    ...
    assert isinstance(opgen, algebra_py.OperationGeneratorPair)
...
def test_freemodule_number_of_gens_in_degree_above_range_returns_zero():
    ...
    fresh = algebra_py.FreeModule(milnor(2), "F", 0)
...
def test_freemodule_has_no_python_mutators():
    ...
    m = algebra_py.FreeModule(milnor(2), "F", 0)
...
def test_fdmodule_build_callable_multiple_times():
    ...
    assert isinstance(sm1, algebra_py.SteenrodModule)
    assert isinstance(sm2, algebra_py.SteenrodModule)
...
def test_module_from_json_prime_mismatch():
    ...
    with pytest.raises((ValueError, RuntimeError)):
        algebra_py.steenrod_module_from_json(milnor(3), C2_JSON)
