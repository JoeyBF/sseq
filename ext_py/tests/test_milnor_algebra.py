...
def make_algebra(p=2, degree=8):
    a = algebra_py.MilnorAlgebra(p)
    a.compute_basis(degree)
    return a


def test_construction_valid_and_invalid_prime():
    a = algebra_py.MilnorAlgebra(2)
    assert a.prime() == 2
    assert algebra_py.MilnorAlgebra(3).prime() == 3

    # A non-prime must raise ValueError, never panic.
    with pytest.raises(ValueError):
        algebra_py.MilnorAlgebra(4)
    with pytest.raises(ValueError):
        algebra_py.MilnorAlgebra(0)
    with pytest.raises(ValueError):
        algebra_py.MilnorAlgebra(1)
...
def test_generic_q_and_profile():
    ...
    a3 = algebra_py.MilnorAlgebra(3)
    assert a3.generic() is True
    assert a3.q() == 4

    profile = a.profile()
    assert isinstance(profile, algebra_py.MilnorProfile)
...
def test_multiply_element_families_via_fpvector():
    ...
    r = fp_py.FpVector.from_slice(2, [1])  # Sq^2 (degree 2, dim 1)
    s = fp_py.FpVector.from_slice(2, [1])  # Sq^2
    out = fp_py.FpVector(2, a.dimension(4))
    a.multiply_element_by_element(out, 1, 2, r, 2, s)
    assert list(out) == [0, 1]

    # multiply_basis_element_by_element with an FpVector element.
    s = fp_py.FpVector.from_slice(2, [1])  # Sq^1
    out = fp_py.FpVector(2, a.dimension(3))
...
def test_multiply_accepts_fpslice_and_fpslicemut():
    ...
    s_vec = fp_py.FpVector.from_slice(2, [1])
    s_slice = s_vec.slice(0, 1)

    result_vec = fp_py.FpVector(2, a.dimension(3))
...
def test_basis_element_from_string_absent_names_raise():
    ...
    profile = algebra_py.MilnorProfile(truncated=True, q_part=0xFFFFFFFF, p_part=[1])
    pa = algebra_py.MilnorAlgebra.new_with_profile(2, profile)
    pa.compute_basis(8)
    with pytest.raises(ValueError):
...
def test_multiply_large_coeff_does_not_overflow():
    ...
    base = fp_py.FpVector(3, a3.dimension(8))
    a3.multiply_basis_elements(base, 1, 4, 0, 4, 0)
    scaled = fp_py.FpVector(3, a3.dimension(8))
...
def test_element_to_string():
    a = make_algebra(2, 6)
    v = fp_py.FpVector.from_slice(2, [1, 1])  # P(3) + P(0, 1) in degree 3
    ...
    with pytest.raises(ValueError):
        a.element_to_string(3, fp_py.FpVector(2, 5))
...
def test_basis_element_index_roundtrip():
    ...
    for d in range(7):
        for i in range(a.dimension(d)):
            ...
            assert isinstance(elt, algebra_py.MilnorBasisElement)
...
def test_basis_element_to_index_not_found():
    a = make_algebra(2, 6)
    bogus = algebra_py.MilnorBasisElement([99], q_part=0, degree=2)
...
def test_milnor_basis_element_fields():
    elt = algebra_py.MilnorBasisElement([2, 1], q_part=3, degree=10)
    ...
    e = algebra_py.MilnorBasisElement([1])
    e.compute_degree(2)
    assert e.degree == 1

    assert algebra_py.MilnorBasisElement([1]) == algebra_py.MilnorBasisElement([1])


def test_milnor_profile_fields_and_methods():
    profile = algebra_py.MilnorProfile(truncated=True, q_part=0b1111, p_part=[3, 2, 1])
    ...
    default = algebra_py.MilnorProfile()
    assert default.is_trivial()
    assert default.q_part == 0xFFFFFFFF


def test_new_with_profile_valid_and_invalid():
    profile = algebra_py.MilnorProfile(truncated=True, q_part=0xFFFFFFFF, p_part=[2, 1])
    a = algebra_py.MilnorAlgebra.new_with_profile(2, profile)
    a.compute_basis(8)
    assert a.prime() == 2

    # An invalid profile raises ValueError instead of panicking.
    bad = algebra_py.MilnorProfile(truncated=True, q_part=0xFFFFFFFF, p_part=[1, 5])
    if not bad.is_valid():
        with pytest.raises(ValueError):
            algebra_py.MilnorAlgebra.new_with_profile(2, bad)
