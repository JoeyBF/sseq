...
def test_reduce_matrix_values_with_nontrivial_quotient():
    ...
    target = fp_py.Subquotient(3, 2)
    target.quotient(fp_py.FpVector.from_slice(3, [0, 1]))
    target.add_gen(fp_py.FpVector.from_slice(3, [1, 0]))

    # Non-identity matrix. `Matrix.apply` computes (input row-vector) * matrix:
    # result = sum_i input[i] * row(i). So gen [1,0] -> row 0 = [2, 1];
    # gen [0,1] -> row 1 = [0, 1].
    m = fp_py.Matrix.from_vec(3, [[2, 1], [0, 1]])
...
def test_reduce_matrix_dimension_mismatches_raise():
    ...
    bad_rows = fp_py.Matrix.from_vec(3, [[1, 0], [0, 1], [0, 0]])
    with pytest.raises(ValueError):
        fp_py.Subquotient.reduce_matrix(bad_rows, source, target)

    # columns != target.ambient_dimension (3 cols vs ambient 2).
    bad_cols = fp_py.Matrix.from_vec(3, [[1, 0, 0], [0, 1, 0]])
    with pytest.raises(ValueError):
        fp_py.Subquotient.reduce_matrix(bad_cols, source, target)

    # prime mismatch between matrix and subquotients.
    bad_prime = fp_py.Matrix.from_vec(5, [[1, 0], [0, 1]])
    with pytest.raises(ValueError):
        fp_py.Subquotient.reduce_matrix(bad_prime, source, target)
