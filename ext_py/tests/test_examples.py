...
``<module>`` is one of the bound modules ``ext_py``/``algebra_py``/``fp_py``/
``sseq_py``) from each example via the ``ast`` module, and assert that every
...
``ext_py.FDModule`` -> ``ext_py.FDModuleBuilder``) but an example still
...
Note on namespaces: the example scripts frequently write ``ext_py.<Name>`` for
classes/functions that are actually bound on a *submodule* of ``ext_py``
(``algebra_py``/``fp_py``/``sseq_py``), because ``from .ext_py import *`` only
re-exports the submodules and the top-level free functions, not the nested
classes. We therefore resolve an ``ext_py.<Name>`` reference as bound if the
name exists on ``ext_py`` OR on any of its bound submodules. This keeps the
...
MODULES = {
    "ext_py": ext_py,
    "algebra_py": algebra_py,
    "fp_py": fp_py,
    "sseq_py": sseq_py,
}

# Submodules reachable from ext_py; an ``ext_py.<Name>`` reference is satisfied
# if <Name> is bound on ext_py itself or on any of these.
EXT_PY_SUBMODULES = (algebra_py, fp_py, sseq_py)
...
def _is_bound(module_name, attr):
    if hasattr(MODULES[module_name], attr):
        return True
    # ext_py shorthand: classes/functions bound on a re-exported submodule.
    if module_name == "ext_py":
...
def test_fdmodulebuilder_rename_is_enforced():
    ...
    ``ext_py.FDModule`` name must not reappear in examples, and the new
    ``FDModuleBuilder`` must be the bound name."""
    assert hasattr(algebra_py, "FDModuleBuilder")
    assert not hasattr(algebra_py, "FDModule")
    for path in _example_files():
        refs = list(_module_attr_references(path.read_text()))
        assert ("ext_py", "FDModule") not in refs, (
            f"{path.name} still references removed ext_py.FDModule"
        )
