...
def adem(p=2):
    return algebra_py.SteenrodAlgebra.adem(p)
...
def test_tensor_module_dimensions():
    algebra = milnor(2)
    t = algebra_py.TensorModule(make_c2(algebra), make_c2(algebra))
    t.compute_basis(4)
...
def test_tensor_module_action_decomposable():
    ...
    res = fp_py.FpVector(2, t.dimension(1))
...
def test_tensor_module_act_by_element_known_result():
    ...
    op = fp_py.FpVector(2, algebra.dimension(1))
    op[0] = 1
    # input = x0 (x) x0 (the single basis element in degree 0).
    inp = fp_py.FpVector(2, t.dimension(0))
    inp[0] = 1
    res = fp_py.FpVector(2, t.dimension(1))
...
def test_suspension_module_act_by_element_prime_mismatch_raises():
    algebra = milnor(2)
    s = algebra_py.SuspensionModule(make_c2(algebra), 3)
    s.compute_basis(8)
    # Operation vector at the wrong prime.
    op = fp_py.FpVector(3, algebra.dimension(1))
    inp = fp_py.FpVector(2, s.dimension(3))
    inp[0] = 1
    res = fp_py.FpVector(2, s.dimension(4))
    with pytest.raises(ValueError):
...
def test_tensor_module_odd_prime_action_runs():
    ...
    assert t.dimension(1) == 2
    assert t.dimension(2) == 1
    res = fp_py.FpVector(3, t.dimension(1))
    t.act_on_basis(res, 1, 1, 0, 0, 0)
...
def test_derived_modules_string_and_is_unit():
    ...
    s = algebra_py.SuspensionModule(make_c2(algebra), 3)
    s.compute_basis(8)
    z = algebra_py.ZeroModule(algebra, 0)
    rp = algebra_py.RealProjectiveSpace(adem(2), 1, 4)
    rp.compute_basis(6)
...
def test_tensor_module_prime_mismatch_raises():
    ...
    with pytest.raises(ValueError):
        algebra_py.TensorModule(left, right)
...
def test_suspension_module_negative_shift():
    algebra = milnor(2)
    s = algebra_py.SuspensionModule(make_c2(algebra), -2)
    s.compute_basis(4)
    assert s.shift() == -2
...
def test_zero_module_is_empty():
    z = algebra_py.ZeroModule(milnor(2), 0)
    z.compute_basis(8)
...
def test_zero_module_default_min_degree():
    z = algebra_py.ZeroModule(milnor(2))
    assert z.min_degree() == 0
...
def test_rp_action():
    ...
    res = fp_py.FpVector(2, rp.dimension(2))
...
def test_rp_infinite():
    ...
    rp = algebra_py.RealProjectiveSpace(adem(2), 1)
    rp.compute_basis(6)
...
def test_rp_rejects_odd_prime():
    with pytest.raises(ValueError):
        algebra_py.RealProjectiveSpace(milnor(3), 1, 4)


def test_rp_rejects_bad_range():
    with pytest.raises(ValueError):
        algebra_py.RealProjectiveSpace(adem(2), 4, 1)
