...
def make_matrix(rows):
    ...
    p = 5
    m = fp_py.Matrix(p, len(rows), len(rows[0]))
...
def test_row_and_row_slice_read():
    ...
    row0 = rect.row(0)
    assert isinstance(row0, fp_py.FpSlice)
    assert len(row0) == 2
    assert row0[0] == 2
    assert row0[1] == 3
...
def test_row_mut_writes_through_to_parent():
    ...
    rm = rect.row_mut(0)
    assert isinstance(rm, fp_py.FpSliceMut)
...
def test_iter_mut_reflects_in_parent():
    ...
    for r in rect.iter():
        assert isinstance(r, fp_py.FpSlice)
...
def test_add_masked():
    m = fp_py.Matrix(3, 2, 2)
    other = fp_py.Matrix.from_vec(3, [[1, 2], [0, 1]])
    ...
    bad_rows = fp_py.Matrix.from_vec(3, [[1, 2]])
    with pytest.raises(ValueError):
        rect.add_masked(bad_rows, [0, 1])
    # Prime mismatch raises ValueError.
    bad_prime = fp_py.Matrix.from_vec(5, [[1, 2], [0, 1]])
    with pytest.raises(ValueError):
...
def test_augmented_segment_mutates_and_reads_back():
    ...
    seg = m.segment(1, 1)
    assert isinstance(seg, fp_py.MatrixSliceMut)
    assert seg.rows() == 2
...
def test_augmented_row_segment_mut_writes_through():
    ...
    assert isinstance(row, fp_py.FpSliceMut)
    row.set_entry(0, 2)
...
def test_augmented_segment_builds_nontrivial_compute_values():
    ...
    m = fp_py.AugmentedMatrix2(3, 3, [5, 3])
