...
def make_subspace(p, rows):
    s = fp_py.Subspace(p, len(rows[0]))
    for row in rows:
        s.add_vector(fp_py.FpVector.from_slice(p, row))
    return s


def test_construction_and_queries():
    linear = make_subspace(2, [[0, 1, 0], [0, 0, 1]])
    offset = fp_py.FpVector.from_slice(2, [1, 0, 0])
    aff = fp_py.AffineSubspace(offset, linear)
...
def test_offset_and_linear_part_roundtrip():
    ...
    stored = aff.offset()
    assert isinstance(stored, fp_py.FpVector)
    assert list(stored) == [1, 0, 0]

    # linear_part() returns an owned Subspace clone.
    lp = aff.linear_part()
    assert isinstance(lp, fp_py.Subspace)
...
def test_offset_is_reduced():
    ...
    linear = make_subspace(2, [[0, 1, 0], [0, 0, 1]])
    offset = fp_py.FpVector.from_slice(2, [1, 1, 0])
    aff = fp_py.AffineSubspace(offset, linear)
...
def test_contains_membership():
    ...
    assert aff.contains(fp_py.FpVector.from_slice(2, [1, 1, 0]))
    assert fp_py.FpVector.from_slice(2, [1, 1, 0]) in aff

    # [0,1,0] has the wrong first coordinate, so it is not contained.
    assert not aff.contains(fp_py.FpVector.from_slice(2, [0, 1, 0]))
    assert fp_py.FpVector.from_slice(2, [0, 1, 0]) not in aff
...
def test_sum_semantics():
    ...
    b = fp_py.AffineSubspace(
        fp_py.FpVector.from_slice(2, [0, 0, 1]),
    ...
    assert s.contains(fp_py.FpVector.from_slice(2, [1, 1, 1]))
    assert not s.contains(fp_py.FpVector.from_slice(2, [0, 0, 0]))
...
def test_invalid_construction_raises():
    ...
    with pytest.raises(ValueError):
        fp_py.AffineSubspace(fp_py.FpVector.from_slice(2, [1, 0]), linear)

    # Offset prime must match the linear part's prime.
    with pytest.raises(ValueError):
        fp_py.AffineSubspace(fp_py.FpVector.from_slice(3, [1, 0, 0]), linear)


def test_invalid_inputs_raise():
    aff = fp_py.AffineSubspace(
        fp_py.FpVector.from_slice(2, [1, 0, 0]),
        make_subspace(2, [[0, 1, 0]]),
    )
    ...
    with pytest.raises(ValueError):
        aff.contains(fp_py.FpVector.from_slice(3, [1, 0, 0]))
    with pytest.raises(ValueError):
        aff.contains(fp_py.FpVector.from_slice(2, [1, 0]))

    # contains_space / sum: incompatible prime or ambient dimension.
    other_prime = fp_py.AffineSubspace(
        fp_py.FpVector.from_slice(3, [1, 0, 0]),
        make_subspace(3, [[0, 1, 0]]),
    )
    other_dim = fp_py.AffineSubspace(
        fp_py.FpVector.from_slice(2, [1, 0, 0, 0]),
