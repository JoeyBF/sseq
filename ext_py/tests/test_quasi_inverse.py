...
def make_qi():
    ...
    preimage = fp_py.Matrix.from_vec(
        2,
        [
            [1, 0, 1, 1],
            [1, 1, 0, 0],
    ...
    return fp_py.QuasiInverse([0, -1, 1, -1, 2, 3], preimage)
...
def test_apply_known_example():
    qi = make_qi()
    v = fp_py.FpVector.from_slice(2, [1, 1, 0, 0, 1, 0])
    out = fp_py.FpVector(2, 4)
    qi.apply(out, 1, v)
...
def test_apply_dimension_and_prime_mismatch():
    ...
    bad_input = fp_py.FpVector(2, 5)
    with pytest.raises(ValueError):
        qi.apply(out, 1, bad_input)
    # Wrong output length (source_dimension is 4).
    good_input = fp_py.FpVector(2, 6)
    with pytest.raises(ValueError):
        qi.apply(fp_py.FpVector(2, 3), 1, good_input)
    # Prime mismatch on input.
    with pytest.raises(ValueError):
        qi.apply(out, 1, fp_py.FpVector(3, 6))


def test_apply_wrong_type():
    qi = make_qi()
    with pytest.raises(ValueError):
        qi.apply(123, 1, fp_py.FpVector(2, 6))
...
def make_identity_qi():
    ...
    preimage = fp_py.Matrix.from_vec(3, [[1, 0, 0], [0, 1, 0], [0, 0, 1]])
    return fp_py.QuasiInverse([0, 1, 2], preimage)
...
def test_apply_input_target_aliasing_raises_runtimeerror():
    ...
    qi = make_identity_qi()
    v = fp_py.FpVector.from_slice(3, [1, 2, 1])
...
def test_apply_wrong_type_is_valueerror_not_runtimeerror():
    ...
    qi = make_identity_qi()
    good = fp_py.FpVector(3, 3)
    with pytest.raises(ValueError):
        qi.apply(123, 1, good)
    with pytest.raises(ValueError):
        qi.apply(fp_py.Matrix.from_vec(3, [[1, 0, 0]]), 1, good)
...
def test_bytes_roundtrip():
    ...
    restored = fp_py.QuasiInverse.from_bytes(2, data)
...
def test_from_bytes_malformed():
    with pytest.raises(RuntimeError):
        fp_py.QuasiInverse.from_bytes(2, b"\x00\x01\x02")
...
def test_compute_quasi_inverse_from_matrix():
    ...
    padded_cols, m = fp_py.Matrix.augmented_from_vec(3, rows)
    m.row_reduce()
...
def test_compute_quasi_inverse_out_of_range():
    _, m = fp_py.Matrix.augmented_from_vec(3, [[1, 0, 1], [0, 1, 1]])
...
def test_compute_quasi_inverse_requires_row_reduce():
    ...
    # a clean ValueError.
    padded_cols, m = fp_py.Matrix.augmented_from_vec(3, [[1, 0, 1], [0, 1, 1]])
    with pytest.raises(ValueError):
...
def test_inconsistent_image_raises():
    ...
    with pytest.raises(ValueError):
        fp_py.QuasiInverse([0, 1, 2, 3, 0], preimage)
...
def test_pivot_out_of_range_raises():
    ...
    with pytest.raises(ValueError):
        fp_py.QuasiInverse([0, 1, 2, 4], preimage)
...
def test_apply_with_coeff_odd_prime():
    ...
    qi = fp_py.QuasiInverse([0, 1, 2], preimage)
    v = fp_py.FpVector.from_slice(3, [1, 2, 1])
...
def test_none_image_construction_and_roundtrip():
    ...
    qi = fp_py.QuasiInverse(None, preimage)
    ...
    v = fp_py.FpVector.from_slice(2, [1, 0, 1])
    out = fp_py.FpVector(2, 3)
    qi.apply(out, 1, v)
    ...
    restored = fp_py.QuasiInverse.from_bytes(2, qi.to_bytes())
    assert restored.pivots() == [0, 1, 2]
    assert restored.target_dimension() == 3
    out2 = fp_py.FpVector(2, 3)
