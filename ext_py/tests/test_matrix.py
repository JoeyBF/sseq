...
def test_matrix_from_vec_and_identity():
    m = fp_py.Matrix.from_vec(7, [[1, 3, 6], [0, 3, 4]])
    assert m.to_vec() == [[1, 3, 6], [0, 3, 4]]
    assert not m.is_zero()

    ident = fp_py.Matrix.identity(5, 3)
    assert ident.to_vec() == [[1, 0, 0], [0, 1, 0], [0, 0, 1]]


def test_matrix_from_rows_and_from_row():
    r0 = fp_py.FpVector.from_slice(5, [1, 2, 3])
    r1 = fp_py.FpVector.from_slice(5, [4, 0, 1])
    m = fp_py.Matrix.from_rows(5, [r0, r1], 3)
    assert m.to_vec() == [[1, 2, 3], [4, 0, 1]]

    single = fp_py.Matrix.from_row(5, r0, 3)
    assert single.to_vec() == [[1, 2, 3]]


def test_matrix_augmented_from_vec():
    first_source, m = fp_py.Matrix.augmented_from_vec(7, [[1, 3, 6], [0, 3, 4]])
    assert first_source >= 3
    assert m.rows() == 2


def test_prime_is_int():
    m = fp_py.Matrix(5, 1, 1)
...
def test_invalid_prime_and_dims():
    with pytest.raises(ValueError):
        fp_py.Matrix(1, 2, 2)
    with pytest.raises(ValueError):
        fp_py.Matrix.from_vec(4, [[1, 2]])
    with pytest.raises(ValueError):
        fp_py.Matrix.from_vec(7, [[1, 2], [3]])


def test_row_access_and_getitem():
    m = fp_py.Matrix.from_vec(5, [[1, 2, 3], [4, 0, 1]])
    row = m.row(1)
    assert row.prime() == 5
    assert len(row) == 3
...
def test_row_mut_add_slice():
    m = fp_py.Matrix.from_vec(5, [[1, 2, 3]])
    other = fp_py.FpVector.from_slice(5, [1, 1, 1])
    m.row_mut(0).add(other.slice(0, 3), 2)
    assert m.to_vec()[0] == [3, 4, 0]


def test_mutators():
    m = fp_py.Matrix.from_vec(5, [[1, 2], [3, 4]])
    m.swap_rows(0, 1)
...
def test_assign_requires_matching_shape():
    m = fp_py.Matrix.from_vec(5, [[1, 2], [3, 4]])
    other = fp_py.Matrix.from_vec(5, [[1, 1], [1, 1]])
    m.assign(other)
    assert m.to_vec() == [[1, 1], [1, 1]]

    mismatch = fp_py.Matrix(5, 3, 2)
    with pytest.raises(ValueError):
        m.assign(mismatch)
    diff_prime = fp_py.Matrix(7, 2, 2)
    with pytest.raises(ValueError):
        m.assign(diff_prime)


def test_row_reduce_rank():
    m = fp_py.Matrix.from_vec(2, [[1, 1, 0], [0, 1, 1], [1, 0, 1]])
...
def test_trim_and_rotate():
    m = fp_py.Matrix.from_vec(5, [[1, 2, 3], [4, 0, 1], [2, 2, 2]])
    m.trim(0, 2, 1)
    assert m.to_vec() == [[2, 3], [0, 1]]

    n = fp_py.Matrix.from_vec(5, [[1, 0], [2, 0], [3, 0]])
    n.rotate_down(0, 3, 1)
    assert n.to_vec() == [[3, 0], [1, 0], [2, 0]]


def test_bytes_roundtrip():
    m = fp_py.Matrix.from_vec(5, [[1, 2, 3], [4, 0, 2]])
    data = m.to_bytes()
    n = fp_py.Matrix.from_bytes(5, 2, 3, data)
...
def test_row_returns_same_type_as_vector_slice():
    m = fp_py.Matrix.from_vec(5, [[1, 2, 3]])
    v = fp_py.FpVector.from_slice(5, [1, 2, 3])
...
def test_row_len_revalidates_after_column_shrink():
    ...
    m = fp_py.Matrix.from_vec(5, [[1, 2, 3, 4], [5, 6, 7, 8]])
    row = m.row(0)
    rm = m.row_mut(1)
    assert len(row) == 4
    assert len(rm) == 4
