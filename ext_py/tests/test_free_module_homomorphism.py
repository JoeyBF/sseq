...
def free_module_gen_in_degree(algebra, gen_degree):
...
def c2_differential_shift(algebra):
    """f: F -> C2 with degree_shift = 1, F = <g> in degree 1 and f(g) = x0."""
    source = free_module_gen_in_degree(algebra, 1)
    target = algebra_py.steenrod_module_from_json(algebra, C2_JSON)
    hom = algebra_py.FreeModuleHomomorphism(source, target, 1)
...
def test_degree_shift_invariants():
    algebra = milnor(2)
    hom = c2_differential_shift(algebra)
...
def test_add_generators_from_matrix_rows_wrong_column_dim_raises():
    ...
    wide = fp_py.Matrix.from_vec(2, [[0, 0, 0]])
    with pytest.raises(ValueError):
...
def test_target_state_is_shared_not_snapshotted():
    ...
    algebra = milnor(2)
    fd = algebra_py.FDModuleBuilder(algebra, "C2", [1, 1], 0)
    ...
    source = free_module_one_generator(algebra)
    hom = algebra_py.FreeModuleHomomorphism(source, sm, 0)
    row = fp_py.FpVector(2, 1)
    row[0] = 1
