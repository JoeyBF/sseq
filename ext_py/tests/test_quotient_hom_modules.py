...
def test_quotient_module_basic_dimensions():
    algebra = milnor(2)
    q = algebra_py.QuotientModule(make_c2(algebra), 1)
    q.compute_basis(2)
...
def test_quotient_module_truncation_zeroes_above():
    ...
    q = algebra_py.QuotientModule(make_c2(algebra), 0)
    q.compute_basis(2)
    assert q.dimension(0) == 1
...
def test_quotient_module_quotient_vector_and_reduce():
    ...
    elt = fp_py.FpVector(2, q.dimension(1))  # original dim in degree 1 is 1
    elt[0] = 1
    q.quotient(1, elt)
    assert q.dimension(1) == 0
...
def test_quotient_module_old_basis_to_new():
    ...
    new = fp_py.FpVector(2, q.dimension(0))
    old = fp_py.FpVector(2, 1)
    old[0] = 1
...
def test_quotient_module_old_basis_to_new_aliasing_raises_runtimeerror():
    ...
    q.compute_basis(2)
    q.quotient_basis_elements(1, [0])
    v = fp_py.FpVector(2, q.dimension(0))
    v[0] = 1
...
def test_quotient_module_construct_below_min_degree_raises():
    algebra = milnor(2)
    with pytest.raises(ValueError):
        algebra_py.QuotientModule(make_c2(algebra), -5)
...
def test_hom_module_dimensions():
    algebra = milnor(2)
    source = free_one_gen(algebra)
    target = make_c2(algebra)
    hom = algebra_py.HomModule(source, target)
...
def test_hom_module_scalar_action():
    algebra = milnor(2)
    hom = algebra_py.HomModule(free_one_gen(algebra), make_c2(algebra))
    hom.compute_basis(0)
    # The field unit (op degree 0, index 0) fixes a basis element.
    res = fp_py.FpVector(2, hom.dimension(-1))
...
def test_hom_module_prime_mismatch_raises():
    ...
    with pytest.raises(ValueError):
        algebra_py.HomModule(source, target)
...
def test_hom_module_unbounded_target_raises():
    ...
    unbounded = free_one_gen(algebra).into_steenrod_module()
    with pytest.raises(ValueError):
        algebra_py.HomModule(source, unbounded)
