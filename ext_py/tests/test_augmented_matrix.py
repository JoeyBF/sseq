...
def test_import_both_classes():
    assert hasattr(fp_py, "AugmentedMatrix2")
    assert hasattr(fp_py, "AugmentedMatrix3")


def test_construction_and_queries():
    m = fp_py.AugmentedMatrix2(3, 2, [2, 2])
    ...
    m3 = fp_py.AugmentedMatrix3(3, 2, [2, 2, 2])
...
def test_invalid_segment_widths_raise():
    ...
    with pytest.raises(ValueError):
        fp_py.AugmentedMatrix2(3, 2, [2])
    with pytest.raises(ValueError):
        fp_py.AugmentedMatrix2(3, 2, [2, 2, 2])
    with pytest.raises(ValueError):
        fp_py.AugmentedMatrix3(3, 2, [2, 2])
    # Invalid prime.
    with pytest.raises(ValueError):
        fp_py.AugmentedMatrix2(4, 2, [2, 2])


def test_add_identity_and_invalid_segments():
    m = fp_py.AugmentedMatrix2(2, 2, [2, 2])
    m.add_identity(1, 1)
    ...
    wide = fp_py.AugmentedMatrix2(2, 2, [3, 3])
    with pytest.raises(ValueError):
...
def test_row_segment_returns_owned_fpvector():
    ...
    assert isinstance(seg, fp_py.FpVector)
    assert len(seg) == 2
    assert seg[0] == 1
    assert seg[1] == 0
...
def test_into_matrix_returns_matrix_and_consumes():
    ...
    cols = m.columns()
    inner = m.into_matrix()
    assert isinstance(inner, fp_py.Matrix)
    assert inner.rows() == 2
...
def test_compute_quasi_inverses_consumes():
    m = fp_py.AugmentedMatrix3(3, 2, [2, 2, 2])
    m.add_identity(0, 0)
    m.add_identity(1, 1)
    m.add_identity(2, 2)
    m.row_reduce()
    a, b = m.compute_quasi_inverses()
    assert isinstance(a, fp_py.QuasiInverse)
...
def test_augmented_matrix3_compute_kernel_and_quasi_inverses():
    ...
    a, b = m.compute_quasi_inverses()
    assert isinstance(a, fp_py.QuasiInverse)
    assert isinstance(b, fp_py.QuasiInverse)
    assert a.prime() == 3
    assert b.prime() == 3
...
def test_compute_methods_require_row_reduce():
    ...
    m2 = fp_py.AugmentedMatrix2(2, 2, [2, 2])
    with pytest.raises(ValueError):
        m2.compute_kernel()
    with pytest.raises(ValueError):
        m2.compute_image()
    ...
    assert m2.rows() == 2
    m2.row_reduce()
    assert isinstance(m2.compute_kernel(), fp_py.Subspace)
    assert m3.rows() == 2
    m3.row_reduce()
...
def test_repr():
    ...
    m3 = fp_py.AugmentedMatrix3(2, 1, [1, 1, 1])
