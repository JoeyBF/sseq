...
def test_fp_module_generators_is_free_module():
    ...
    m = a_mod_sq1(algebra)
    gens = m.generators()
    assert isinstance(gens, algebra_py.FreeModule)
...
def test_fp_module_into_steenrod_module_round_trip():
    ...
    assert isinstance(boxed, algebra_py.SteenrodModule)
...
def test_fp_module_is_immutable():
    ...
    with pytest.raises(TypeError):
        algebra_py.FPModule(algebra, "M", 0)
...
def test_free_module_has_no_mutators():
    ...
    assert not hasattr(algebra_py.FreeModule, "add_generators")
    assert not hasattr(algebra_py.FreeModule, "extend_by_zero")
...
def test_fp_module_builder_build_and_mutation_after_build_raises():
    algebra = milnor(2)
    b = algebra_py.FPModuleBuilder(algebra, "M", 0)
    assert b.prime() == 2
    assert b.min_degree() == 0
    b.add_generators(0, ["x0"])
    m = b.build()
    assert isinstance(m, algebra_py.FPModule)
...
def test_fp_module_bad_prime_raises():
    with pytest.raises(ValueError):
        algebra_py.SteenrodAlgebra.milnor(4)
...
def test_fp_module_from_json():
    algebra = milnor(2)
    m = algebra_py.FPModule.from_json(algebra, A_MOD_SQ1_SQ2)
    assert isinstance(m, algebra_py.FPModule)
    assert m.prime() == 2
    assert m.min_degree() == 0
    assert m.dimension(0) == 1
...
def test_fp_module_from_json_prime_mismatch_raises():
    algebra = milnor(3)
    with pytest.raises(ValueError):
        algebra_py.FPModule.from_json(algebra, A_MOD_SQ1_SQ2)
...
def test_block_structure_queries():
    ...
    bs = algebra_py.BlockStructure(0, [[2, 1], [3]])
    ...
    assert isinstance(pair, algebra_py.GeneratorBasisEltPair)
...
def test_block_structure_add_block():
    bs = algebra_py.BlockStructure(0, [[2, 1], [3]])
    target = fp_py.FpVector(2, bs.total_dimension())
    source = fp_py.FpVector(2, 3)
...
def test_generator_basis_elt_pair_construct():
    p = algebra_py.GeneratorBasisEltPair(3, 1, 2)
