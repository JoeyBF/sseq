...
def c2_module(algebra):
    return algebra_py.steenrod_module_from_json(algebra, C2_JSON)
...
def test_full_module_homomorphism_in_module():
    assert "FullModuleHomomorphism" in dir(algebra_py)
...
def test_zero_construct_and_invariants():
    algebra = milnor(2)
    m = c2_module(algebra)
    hom = algebra_py.FullModuleHomomorphism(m, m, 0)
...
def test_source_and_target_types_and_state():
    ...
    source = hom.source()
    target = hom.target()
    assert isinstance(source, algebra_py.SteenrodModule)
...
def test_construct_requires_same_algebra():
    ...
    with pytest.raises(ValueError):
        algebra_py.FullModuleHomomorphism(m1, m2, 0)
...
def test_zero_maps_everything_to_zero():
    algebra = milnor(2)
    m = c2_module(algebra)
    hom = algebra_py.FullModuleHomomorphism.zero(m, m, 0)
    assert isinstance(hom, algebra_py.FullModuleHomomorphism)
...
def test_identity_is_identity():
    algebra = milnor(2)
    m = c2_module(algebra)
    hom = algebra_py.FullModuleHomomorphism.identity(m)
    assert hom.degree_shift() == 0
    # identity: x0 -> x0 (degree 0), x1 -> x1 (degree 1).
    res0 = fp_py.FpVector(2, 1)
...
def test_identity_requires_bounded_module():
    algebra = milnor(2)
    free = free_module_one_generator(algebra)
    free.compute_basis(2)
    unbounded = free.into_steenrod_module()
    with pytest.raises(ValueError):
        algebra_py.FullModuleHomomorphism.identity(unbounded)


# --- from_matrices ---------------------------------------------------------


def from_matrices_bottom_cell(algebra):
    """f: C2 -> C2, degree_shift 0: identity on the bottom cell (degree 0),
    zero on the top cell (degree 1)."""
    m = c2_module(algebra)
    m0 = fp_py.Matrix.from_vec(2, [[1]])
    m1 = fp_py.Matrix.from_vec(2, [[0]])
    return algebra_py.FullModuleHomomorphism.from_matrices(m, m, [m0, m1], 0)


def test_from_matrices_known_values():
    algebra = milnor(2)
    hom = from_matrices_bottom_cell(algebra)
    # apply_to_basis_element: degree 0 -> [1], degree 1 -> [0].
    r0 = fp_py.FpVector(2, 1)
...
def test_from_matrices_dimension_mismatch_raises():
    ...
    bad = fp_py.Matrix.from_vec(2, [[1, 0]])
    with pytest.raises(ValueError):
        algebra_py.FullModuleHomomorphism.from_matrices(m, m, [bad], 0)


def test_from_matrices_prime_mismatch_raises():
    algebra = milnor(2)
    m = c2_module(algebra)
    bad = fp_py.Matrix.from_vec(3, [[1]])
    with pytest.raises(ValueError):
        algebra_py.FullModuleHomomorphism.from_matrices(m, m, [bad], 0)
...
def test_auxiliary_data_dimensions_and_types():
    ...
    assert isinstance(image0, fp_py.Subspace)
    assert isinstance(kernel0, fp_py.Subspace)
    assert isinstance(qi0, fp_py.QuasiInverse)
...
def test_apply_out_of_range_index_raises():
    algebra = milnor(2)
    hom = algebra_py.FullModuleHomomorphism.identity(c2_module(algebra))
    res = fp_py.FpVector(2, 1)
...
def test_from_matrices_rejects_min_degree_below_target_min():
    ...
    with pytest.raises(ValueError):
        algebra_py.FullModuleHomomorphism.from_matrices(
...
def test_from_matrices_min_degree_at_target_min_and_default_ok():
    ...
    hom = algebra_py.FullModuleHomomorphism.from_matrices(
        m, m, [], 0, min_degree=target_min
    )
    assert isinstance(hom, algebra_py.FullModuleHomomorphism)
    # The default (None) path is unaffected.
    hom_default = algebra_py.FullModuleHomomorphism.from_matrices(m, m, [], 0)
    assert isinstance(hom_default, algebra_py.FullModuleHomomorphism)
...
def test_from_matrices_explicit_min_degree_multi_degree_apply():
    ...
    # below would flip.
    m0 = fp_py.Matrix.from_vec(2, [[0]])  # output degree 0
    m1 = fp_py.Matrix.from_vec(2, [[1]])  # output degree 1
    hom = algebra_py.FullModuleHomomorphism.from_matrices(
...
def shift_one_top_to_bottom(algebra):
    ...
    m = c2_module(algebra)
    m0 = fp_py.Matrix.from_vec(2, [[1]])
    return algebra_py.FullModuleHomomorphism.from_matrices(m, m, [m0], 1)


def test_shift_apply_lands_in_shifted_degree():
    algebra = milnor(2)
    hom = shift_one_top_to_bottom(algebra)
...
def test_shift_get_partial_matrix_success_and_guard():
    ...
    assert isinstance(gm, fp_py.Matrix)
    assert gm.rows() == 1
    assert gm.columns() == 1
    assert gm.to_vec() == [[1]]
...
def test_shift_from_matrices_row_dimension_validation():
    ...
    bad = fp_py.Matrix.from_vec(2, [[1], [1]])
    with pytest.raises(ValueError):
        algebra_py.FullModuleHomomorphism.from_matrices(m, m, [bad], 1)
...
def test_apply_output_degree_overflow_raises():
    ...
    hom = algebra_py.FullModuleHomomorphism(m, m, -2147483648)
    res = fp_py.FpVector(2, 1)
    with pytest.raises(ValueError):
        hom.apply_to_basis_element(res, 1, 0, 0)
