...
def make(variant, p=2, degree=8):
    if variant == "adem":
        a = algebra_py.SteenrodAlgebra.adem(p)
    else:
        a = algebra_py.SteenrodAlgebra.milnor(p)
...
def test_construction_and_variant():
    adem = algebra_py.SteenrodAlgebra.adem(2)
    assert adem.prime() == 2
    assert adem.algebra_type() == algebra_py.AlgebraType.Adem

    milnor = algebra_py.SteenrodAlgebra.milnor(3)
    assert milnor.prime() == 3
    assert milnor.algebra_type() == algebra_py.AlgebraType.Milnor


def test_invalid_prime_raises():
    for bad in (4, 0, 1):
        with pytest.raises(ValueError):
            algebra_py.SteenrodAlgebra.adem(bad)
        with pytest.raises(ValueError):
            algebra_py.SteenrodAlgebra.milnor(bad)


def test_prime_is_plain_int():
    a = algebra_py.SteenrodAlgebra.milnor(2)
    p = a.prime()
...
def test_multiply_element_families_via_fpvector(variant):
    ...
    r = fp_py.FpVector.from_slice(2, [1])  # Sq1 (degree 1, dim 1)
    s = fp_py.FpVector.from_slice(2, [1])  # Sq2 (degree 2, dim 1)
...
def test_multiply_accepts_fpslice_and_fpslicemut(variant):
    ...
    s_vec = fp_py.FpVector.from_slice(2, [1])  # Sq2
...
def test_element_to_string(variant):
    a = make(variant, 2, 8)
    v = fp_py.FpVector.from_slice(2, [1, 1])  # degree 3 has dim 2
...
def test_from_json_constructs_known_algebra():
    spec = {"p": 2}
    adem = algebra_py.SteenrodAlgebra.from_json(spec, algebra_py.AlgebraType.Adem, False)
    assert adem.prime() == 2
    assert adem.algebra_type() == algebra_py.AlgebraType.Adem

    milnor = algebra_py.SteenrodAlgebra.from_json(spec, algebra_py.AlgebraType.Milnor, False)
    assert milnor.prime() == 2
    assert milnor.algebra_type() == algebra_py.AlgebraType.Milnor

    # from_json defaults unstable to False.
    again = algebra_py.SteenrodAlgebra.from_json(spec, algebra_py.AlgebraType.Adem)
    ...
    fallback = algebra_py.SteenrodAlgebra.from_json(
        listed, algebra_py.AlgebraType.Adem, False
    )
    assert fallback.algebra_type() == algebra_py.AlgebraType.Milnor
...
def test_from_json_bad_prime_raises():
    ...
    with pytest.raises(RuntimeError):
        algebra_py.SteenrodAlgebra.from_json({"p": 4}, algebra_py.AlgebraType.Adem, False)


def test_from_json_rejects_non_dict():
    # A list converts to JSON fine, but upstream rejects the shape -> RuntimeError.
    with pytest.raises(RuntimeError):
        algebra_py.SteenrodAlgebra.from_json([1, 2, 3], algebra_py.AlgebraType.Adem, False)
...
def test_from_json_large_int_in_spec():
    ...
    with pytest.raises(RuntimeError):
        algebra_py.SteenrodAlgebra.from_json({"p": big}, algebra_py.AlgebraType.Adem, False)

    # An int outside [i64::MIN, u64::MAX] is rejected by py_to_json itself with
    # a ValueError (taxonomy), not OverflowError.
    too_big = 2**64
    with pytest.raises(ValueError):
        algebra_py.SteenrodAlgebra.from_json(
            {"p": too_big}, algebra_py.AlgebraType.Adem, False
        )
