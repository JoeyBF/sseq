...
def make_algebra(p=2, degree=8):
    a = algebra_py.AdemAlgebra(p)
    a.compute_basis(degree)
    return a


def test_construction_valid_and_invalid_prime():
    a = algebra_py.AdemAlgebra(2)
    assert a.prime() == 2
    assert algebra_py.AdemAlgebra(3).prime() == 3

    # A non-prime must raise ValueError, never panic.
    for bad in (4, 0, 1):
        with pytest.raises(ValueError):
            algebra_py.AdemAlgebra(bad)
...
def test_multiply_element_families_via_fpvector():
    ...
    r = fp_py.FpVector.from_slice(2, [1])  # Sq2 (degree 2, dim 1)
    ...
    s = fp_py.FpVector.from_slice(2, [1])  # Sq2
    ...
    r = fp_py.FpVector.from_slice(2, [1])  # Sq1
    out = fp_py.FpVector(2, a.dimension(3))
...
def test_multiply_input_target_aliasing_raises_runtimeerror():
    ...
    a = make_algebra(2, 8)
    v = fp_py.FpVector.from_slice(2, [1])  # Sq1 element / result alias
...
def test_basis_element_index_roundtrip():
    ...
    for d in range(9):
        for i in range(a.dimension(d)):
            ...
            assert isinstance(elt, algebra_py.AdemBasisElement)
...
def test_basis_element_to_index_not_found():
    ...
    bogus = algebra_py.AdemBasisElement([1, 2], degree=3)
...
def test_adem_basis_element_fields():
    elt = algebra_py.AdemBasisElement([2, 1], bocksteins=1, degree=10, p_or_sq=True)
    assert elt.ps == [2, 1]
    ...
    assert algebra_py.AdemBasisElement([1]) == algebra_py.AdemBasisElement([1])
    assert algebra_py.AdemBasisElement([1]) != algebra_py.AdemBasisElement([2])


def test_por_bockstein_variants():
    p = algebra_py.PorBockstein.P(3)
    b = algebra_py.PorBockstein.Bockstein(True)
    assert isinstance(p, algebra_py.PorBockstein)
    assert isinstance(b, algebra_py.PorBockstein)
    ...
    assert all(isinstance(x, algebra_py.PorBockstein) for x in pieces)
    assert any(isinstance(x, algebra_py.PorBockstein.Bockstein) for x in pieces)
