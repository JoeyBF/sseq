...
def test_subspace_entire_space():
    s = fp_py.Subspace.entire_space(2, 3)
    assert s.dimension() == 3
    assert s.ambient_dimension() == 3


def test_subspace_from_matrix():
    m = fp_py.Matrix.from_vec(3, [[1, 0, 0], [0, 1, 2]])
    s = fp_py.Subspace.from_matrix(m)
...
def test_add_vector_and_contains():
    s = fp_py.Subspace(3, 3)
    v = fp_py.FpVector.from_slice(3, [1, 0, 0])
    assert s.add_vector(v) == 1
    assert s.contains(v)
    assert v in s

    w = fp_py.FpVector.from_slice(3, [0, 1, 0])
    assert not s.contains(w)
    assert w not in s

    # A scalar multiple is still contained.
    twice = fp_py.FpVector.from_slice(3, [2, 0, 0])
    assert s.contains(twice)


def test_contains_space_and_sum():
    a = fp_py.Subspace(3, 3)
    a.add_vector(fp_py.FpVector.from_slice(3, [1, 0, 0]))
    b = fp_py.Subspace(3, 3)
    b.add_vector(fp_py.FpVector.from_slice(3, [0, 1, 0]))

    assert not a.contains_space(b)

    # Incompatible prime/ambient dimension raise ValueError rather than panic.
    with pytest.raises(ValueError):
        a.contains_space(fp_py.Subspace(5, 3))
    with pytest.raises(ValueError):
        a.contains_space(fp_py.Subspace(3, 4))
    ...
    assert s.contains(fp_py.FpVector.from_slice(3, [1, 0, 0]))
    assert s.contains(fp_py.FpVector.from_slice(3, [0, 1, 0]))
    assert s.dimension() <= s.ambient_dimension()

    # Overlapping subspaces: the sum's dimension is the union's rank.
    c = fp_py.Subspace(3, 3)
    c.add_vector(fp_py.FpVector.from_slice(3, [1, 0, 0]))
    overlap = a.sum(c)
...
def test_reduce_in_place():
    s = fp_py.Subspace(3, 3)
    s.add_vector(fp_py.FpVector.from_slice(3, [1, 0, 0]))

    v = fp_py.FpVector.from_slice(3, [2, 1, 0])
    s.reduce(v)
    assert v[0] == 0
    assert v[1] == 1
...
def test_bytes_roundtrip():
    ...
    restored = fp_py.Subspace.from_bytes(3, data)
    assert restored.dimension() == 1
    assert restored.contains(fp_py.FpVector.from_slice(3, [1, 0, 0]))
...
def test_invalid_inputs_raise():
    ...
    with pytest.raises(ValueError):
        s.contains(fp_py.FpVector.from_slice(5, [1, 0, 0]))

    with pytest.raises(ValueError):
        s.add_vector(fp_py.FpVector.from_slice(3, [1, 0]))
...
def test_bytes_error_taxonomy_is_consistent():
    ...
    with pytest.raises(RuntimeError):
        fp_py.Subspace.from_bytes(3, b"\x00\x01\x02")
