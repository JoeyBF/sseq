# Python API Proposal for `ext_py`

This document specifies the Python API for `ext_py`, the PyO3 bindings to the `ext` workspace.
...
```python
import ext_py
from ext_py import fp_py, algebra_py, sseq_py
# ext-crate items (Resolution, query_module, …) live at the top level of ext_py
```

`fp_py`, `algebra_py`, `sseq_py` are PyO3 submodules; the `ext` crate's own surface sits directly in
`ext_py`. Below, each section header notes its Python home.
...
| `power_mod(p, b, e)` | `fp_py.power_mod(p, b, e)` |
| `log2(n)`, `logp(p, n)`, `factor_pk(p, n)`, `inverse(p, k)`, `minus_one_to_the_n(p, i)`, `is_prime(p)` | module-level `fp_py.*` functions |
| `Binomial::{binomial, multinomial, binomial_odd_is_zero, …}` | module-level `fp_py.binomial(p, n, k)`, `fp_py.multinomial(p, list)`, … (take prime + ints) |
| `TWO` constant | `fp_py.TWO` |
...
| `F2` (and `F3/F5/F7` under `odd-primes`) | `fp_py.F2`, … |
...
## 5. `algebra` crate  →  `ext_py.algebra_py`
...
| `Field::new(p)` | `algebra_py.Field(p)` (the trivial 1-dim algebra; distinct from `fp_py.Fp`) |
| `AlgebraType` enum | already bound; keep `MILNOR`/`ADEM` |
| `module_gens_from_json(value)` | `algebra_py.module_gens_from_json(dict)` → `(graded_dims, names)` (the returned closure is dropped; not bindable, see §8) |
...
### 5.3 Modules (`algebra::module`)  →  `algebra_py`
...
| `from_json(algebra, value)` (steenrod_module) | `algebra_py.steenrod_module_from_json(algebra, dict)` |
...
### 5.4 Module homomorphisms (`algebra::module::homomorphism`)  →  `algebra_py`
...
### 5.5 Steenrod evaluator / parser  →  `algebra_py`
...
| `parse_algebra(str)`, `parse_module(str)` | `algebra_py.parse_algebra`, `parse_module` returning the parse-tree pyclasses |
...
| `iter_s_t(f, min, max)` | `sseq_py.iter_s_t(callback, min, max)` |
...
| `Orientation` enum | `sseq_py.Orientation.{Left, Right, Above, Below}` |
...
| `LAMBDA_BIDEGREE` constant | `ext_py.LAMBDA_BIDEGREE` |

### 7.5 `ext::yoneda`

| Rust item | Python binding |
|---|---|
| `yoneda_representative_element(cc, b, class)` | `ext_py.yoneda_representative_element(resolution, bidegree, class)` (used by `steenrod.py`) |
| `yoneda_representative(cc, map)` | `ext_py.yoneda_representative(resolution, chain_map)` |
...
| `construct(spec, save_dir)` | `ext_py.construct(spec, save_dir=None)` (spec = dict or `"S_2"` string) |
| `construct_standard`, `construct_nassau` | `construct_standard`, `construct_nassau` |
| `parse_module_name(name)` | `ext_py.parse_module_name(name) -> dict` |
| `load_module_json(name)` | `ext_py.load_module_json(name) -> dict` |
| `get_unit(resolution)` | `ext_py.get_unit(resolution) -> (is_unit, unit_resolution)` (used by `massey.py`) |
| `unicode_num(n)`, `secondary_job()` | module-level helpers |
| `init_logging()` | already called in `#[pymodule_init]`; also expose `ext_py.init_logging()` |
| `Config` struct | `ext_py.Config(module, algebra)` pyclass |
...
1. `fp_py`: `ValidPrime`, `FpVector`, `Matrix`, `Subspace` (foundation for everything).
2. `algebra_py`: `SteenrodAlgebra`, `MilnorAlgebra`, `AdemAlgebra`, the `Algebra` method set,
   `SteenrodModule` + `FDModuleBuilder`/`FreeModule`/`TensorModule`, `steenrod_module_from_json`.
3. `sseq_py`: `Bidegree`, `BidegreeGenerator`, `BidegreeElement`, `Sseq`, `SvgBackend`.
4. `ext_py` top level: `Resolution`, `construct`/`query_module*`, `ChainComplex` method set,
