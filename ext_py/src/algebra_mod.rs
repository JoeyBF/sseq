...
pub mod algebra_py {
    ...
    fn module_act_on_basis<A: Algebra>(
        ...
        checked_mod_index(m, mod_degree, mod_index)?;
        crate::fp_py::with_target_slice_mut(py, result, |mut res| {
            checked_same_prime(res.prime().as_u32(), p)?;
    ...
    mod tests {
        ...
        /// The C2 module: x0 in degree 0, x1 in degree 1, Sq1 x0 = x1.
        const C2_JSON: &str = r#"{
            "p": 2,
        ...
        fn c2_module(py: Python<'_>) -> SteenrodModule {
            let algebra = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
            let json: serde_json::Value = serde_json::from_str(C2_JSON).unwrap();
            let arc = algebra.borrow(py).arc();
            let m = steenrod_module::from_json(arc, &json)
                .map(SteenrodModule)
                .unwrap();
            m
        }

        /// A C2 `SteenrodModule` built over the given (shared) algebra, so two
        /// such factors share the same `Arc<SteenrodAlgebra>`.
        fn c2_module_over(py: Python<'_>, algebra: &Py<SteenrodAlgebra>) -> SteenrodModule {
            let json: serde_json::Value = serde_json::from_str(C2_JSON).unwrap();
            steenrod_module::from_json(algebra.borrow(py).arc(), &json)
                .map(SteenrodModule)
                .unwrap()
        }

        #[test]
        fn fdmodule_construct_and_invariants() {
            Python::initialize();
            Python::attach(|py| {
                let algebra = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
                ...
                assert_eq!(m.dimension(1), 1);
                assert_eq!(m.dimension(2), 0);
                assert_eq!(m.dimension(-1), 0);
                assert_eq!(m.max_degree(), Some(1));
                assert_eq!(m.total_dimension().unwrap(), 2);

                // Set Sq1 x0 = x1 (Sq1 is op (degree 1, index 0)).
                m.set_action(1, 0, 0, 0, vec![1]).unwrap();
                assert_eq!(m.action(1, 0, 0, 0).unwrap(), vec![1]);

                // act_on_basis: Sq1 . x0 = x1.
                let res = Py::new(
                    py,
                    crate::fp_py::PyFpVector::new(2, m.dimension(1)).unwrap(),
                )
                .unwrap();
                m.act_on_basis(py, res.bind(py).as_any(), 1, 1, 0, 0, 0)
                    .unwrap();
        ...
        #[test]
        fn fdmodule_set_action_out_of_range_raises() {
            Python::initialize();
            Python::attach(|py| {
                let algebra = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
                let mut m =
                ...
                assert!(m.set_action(1, 0, 0, 5, vec![1]).is_err());
            });
        }

        #[test]
        fn steenrod_module_from_json_c2() {
            Python::initialize();
            Python::attach(|py| {
                let m = c2_module(py);
                assert_eq!(m.prime(), 2);
                ...
                assert_eq!(m.dimension(1), 1);
                assert_eq!(m.dimension(2), 0);
                assert_eq!(m.basis_element_to_string(0, 0).unwrap(), "x0");
            });
        }
        ...
        #[test]
        fn freemodule_add_generators_and_opgen() {
            Python::initialize();
            Python::attach(|py| {
                let algebra = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
                ...
                assert_eq!(m.dimension(0), 1);
                assert_eq!(m.dimension(1), 1); // Sq1
                assert_eq!(m.dimension(2), 1); // Sq2
                assert_eq!(m.dimension(3), 2); // Sq3, Sq2 Sq1

                // index_to_op_gen of basis element in degree 1 is (Sq1, gen 0).
                let opgen = m.index_to_op_gen(1, 0).unwrap();
                assert_eq!(opgen.generator_degree(), 0);
                assert_eq!(opgen.generator_index(), 0);
                assert_eq!(opgen.operation_degree(), 1);
        ...
        #[test]
        fn steenrod_module_from_json_prime_mismatch_raises() {
            Python::initialize();
            Python::attach(|py| {
                // A p = 3 algebra against a p = 2 spec must raise, not panic
                // (upstream would index `actions` out of bounds).
                let algebra = Py::new(py, SteenrodAlgebra::milnor(3, false).unwrap()).unwrap();
                let dict = pyo3::types::PyDict::new(py);
                dict.set_item("p", 2).unwrap();
                dict.set_item("type", "finite dimensional module").unwrap();
                let gens = pyo3::types::PyDict::new(py);
                gens.set_item("x0", 0).unwrap();
                gens.set_item("x1", 1).unwrap();
                dict.set_item("gens", gens).unwrap();
                dict.set_item("actions", vec!["Sq1 x0 = x1"]).unwrap();
                match steenrod_module_from_json(algebra.borrow(py), dict.as_any()) {
                    Ok(_) => panic!("expected a prime-mismatch error"),
        ...
        #[test]
        fn freemodule_total_dimension_unbounded_raises() {
            Python::initialize();
            Python::attach(|py| {
                let algebra = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
                let m = FreeModule::new(algebra.borrow(py), "F".to_string(), 0);
        ...
        // --- §5.3 derived / standalone modules --------------------------------

        #[test]
        fn tensor_module_dimensions_and_action() {
            Python::initialize();
            Python::attach(|py| {
                let algebra = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
                let left = Py::new(py, c2_module_over(py, &algebra)).unwrap();
                let right = Py::new(py, c2_module_over(py, &algebra)).unwrap();
                let t = TensorModule::new(left.borrow(py), right.borrow(py)).unwrap();
                t.compute_basis(4);
                assert_eq!(t.prime(), 2);
                assert_eq!(t.min_degree(), 0);
                // C2 (x) C2: dims are the convolution of [1,1] with [1,1].
                assert_eq!(t.dimension(0), 1); // x0.x0
                assert_eq!(t.dimension(1), 2); // x1.x0, x0.x1
                assert_eq!(t.dimension(2), 1); // x1.x1
                assert_eq!(t.dimension(3), 0);
                assert_eq!(t.max_degree(), Some(2));
                assert_eq!(t.total_dimension().unwrap(), 4);

                // seek_module_num / offset thin extras.
                // In total degree 1, index 0 has left factor in degree 0 (or 1).
                assert!(t.seek_module_num(1, 0).is_ok());
                assert!(t.offset(1, 0).is_ok());
                // Out-of-range basis index / left_degree raise.
                assert!(t.seek_module_num(1, 9).is_err());
                assert!(t.offset(1, 9).is_err());

                // Sq1 . (x0 (x) x0) = x1 (x) x0 + x0 (x) x1 (both entries 1 at p=2).
                let res = Py::new(
                    py,
                    crate::fp_py::PyFpVector::new(2, t.dimension(1)).unwrap(),
                )
                .unwrap();
                t.act_on_basis(py, res.bind(py).as_any(), 1, 1, 0, 0, 0)
                    .unwrap();
                assert_eq!(res.borrow(py).entry(0).unwrap(), 1);
        ...
        #[test]
        fn tensor_module_prime_mismatch_raises() {
            Python::initialize();
            Python::attach(|py| {
                // Two C2 modules over algebras at different primes must raise.
                let alg3 = Py::new(py, SteenrodAlgebra::milnor(3, false).unwrap()).unwrap();
                let zero3 = ZeroModule::new(alg3.borrow(py), 0);
                let left = Py::new(py, zero3.into_steenrod_module()).unwrap();
                let right = Py::new(py, c2_module(py)).unwrap();
                assert!(TensorModule::new(left.borrow(py), right.borrow(py)).is_err());

                // Same prime but distinct algebra objects also raise.
                let alg_a = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
                let alg_b = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
                let za = Py::new(
                    py,
                    ZeroModule::new(alg_a.borrow(py), 0).into_steenrod_module(),
                )
                .unwrap();
                let zb = Py::new(
                    py,
                    ZeroModule::new(alg_b.borrow(py), 0).into_steenrod_module(),
                )
                .unwrap();
                assert!(TensorModule::new(za.borrow(py), zb.borrow(py)).is_err());
            });
        }

        #[test]
        fn suspension_module_shifts_degrees() {
            Python::initialize();
            Python::attach(|py| {
                let inner = Py::new(py, c2_module(py)).unwrap();
                let s = SuspensionModule::new(inner.borrow(py), 3);
                s.compute_basis(8);
                assert_eq!(s.shift(), 3);
                assert_eq!(s.prime(), 2);
                assert_eq!(s.min_degree(), 3);
                assert_eq!(s.dimension(0), 0);
                assert_eq!(s.dimension(3), 1); // shifted x0
                assert_eq!(s.dimension(4), 1); // shifted x1
                assert_eq!(s.dimension(5), 0);
                assert_eq!(s.max_degree(), Some(4));

                // Suspension preserves the inner action: Sq1 . (shifted x0) = shifted x1.
                let res = Py::new(
                    py,
                    crate::fp_py::PyFpVector::new(2, s.dimension(4)).unwrap(),
                )
                .unwrap();
                s.act_on_basis(py, res.bind(py).as_any(), 1, 1, 0, 3, 0)
                    .unwrap();
                assert_eq!(res.borrow(py).entry(0).unwrap(), 1);
            });
        }
        ...
        #[test]
        fn zero_module_is_empty() {
            Python::initialize();
            Python::attach(|py| {
                let algebra = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
                let z = ZeroModule::new(algebra.borrow(py), 0);
                z.compute_basis(8);
                assert_eq!(z.prime(), 2);
                assert_eq!(z.min_degree(), 0);
                for d in -2..=8 {
                    assert_eq!(z.dimension(d), 0);
                }
                assert_eq!(z.total_dimension().unwrap(), 0);
                // Round-trips into a SteenrodModule that is also empty.
                let boxed = z.into_steenrod_module();
                assert_eq!(boxed.dimension(0), 0);
            });
        }

        #[test]
        fn real_projective_space_dimensions_and_action() {
            Python::initialize();
            Python::attach(|py| {
                let algebra = Py::new(py, SteenrodAlgebra::adem(2, false).unwrap()).unwrap();
                let rp = RealProjectiveSpace::new(algebra.borrow(py), 1, Some(4), false).unwrap();
                rp.compute_basis(6);
                assert_eq!(rp.prime(), 2);
                assert_eq!(rp.min_degree(), 1);
                assert_eq!(rp.min(), 1);
                assert_eq!(rp.max(), Some(4));
                assert!(!rp.clear_bottom());
                for d in 1..=4 {
                    assert_eq!(rp.dimension(d), 1);
                }
                assert_eq!(rp.dimension(0), 0);
                assert_eq!(rp.dimension(5), 0);
                assert_eq!(rp.total_dimension().unwrap(), 4);

                // Sq1 . x^1 = x^2 (binomial(1, 1) = 1 mod 2).
                let res = Py::new(
                    py,
                    crate::fp_py::PyFpVector::new(2, rp.dimension(2)).unwrap(),
                )
                .unwrap();
                rp.act_on_basis(py, res.bind(py).as_any(), 1, 1, 0, 1, 0)
                    .unwrap();
                assert_eq!(res.borrow(py).entry(0).unwrap(), 1);
            });
        }
        ...
        #[test]
        fn real_projective_space_rejects_odd_prime_and_bad_range() {
            Python::initialize();
            Python::attach(|py| {
                let alg3 = Py::new(py, SteenrodAlgebra::milnor(3, false).unwrap()).unwrap();
                assert!(RealProjectiveSpace::new(alg3.borrow(py), 1, Some(4), false).is_err());
                let alg2 = Py::new(py, SteenrodAlgebra::adem(2, false).unwrap()).unwrap();
                // max < min raises.
                assert!(RealProjectiveSpace::new(alg2.borrow(py), 4, Some(1), false).is_err());
            });
        }

        #[test]
        fn derived_modules_out_of_range_no_panic() {
            Python::initialize();
            Python::attach(|py| {
                let algebra = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
                let left = Py::new(py, c2_module_over(py, &algebra)).unwrap();
                let right = Py::new(py, c2_module_over(py, &algebra)).unwrap();
                let t = TensorModule::new(left.borrow(py), right.borrow(py)).unwrap();
                // Negative / above-range degrees read as empty, never panic.
                assert_eq!(t.dimension(-5), 0);
                assert_eq!(t.dimension(100), 0);
                assert!(t.basis_element_to_string(100, 0).is_err());
                assert!(t.basis_element_to_string(-1, 0).is_err());
            });
        }
        ...
        #[test]
        fn quotient_module_dimensions_and_quotienting() {
            Python::initialize();
            Python::attach(|py| {
                // C2 (dims [1, 1], Sq1 x0 = x1), truncated at degree 1.
                let inner = Py::new(py, c2_module(py)).unwrap();
                let q = QuotientModule::new(inner.borrow(py), 1).unwrap();
                q.compute_basis(2);
                assert_eq!(q.prime(), 2);
                assert_eq!(q.min_degree(), 0);
                assert_eq!(q.truncation(), 1);
                // No subspace quotiented yet: same dims as C2.
                assert_eq!(q.dimension(0), 1);
                assert_eq!(q.dimension(1), 1);
                assert_eq!(q.dimension(2), 0);
                assert_eq!(q.max_degree(), Some(1));
                assert_eq!(q.total_dimension().unwrap(), 2);

                // Quotient out x1 (the only basis element in degree 1).
                let mut q = q;
                q.quotient_basis_elements(1, vec![0]).unwrap();
                assert_eq!(q.dimension(1), 0);
                assert_eq!(q.dimension(0), 1);
                assert_eq!(q.total_dimension().unwrap(), 1);

                // old_basis_to_new in degree 0: x0 survives, so [1] -> [1].
                let new = Py::new(py, crate::fp_py::PyFpVector::new(2, 1).unwrap()).unwrap();
                let old = Py::new(
                    py,
                    crate::fp_py::PyFpVector::from_slice(2, vec![1]).unwrap(),
                )
                .unwrap();
                q.old_basis_to_new(py, 0, new.bind(py).as_any(), old.bind(py).as_any())
                    .unwrap();
                assert_eq!(new.borrow(py).entry(0).unwrap(), 1);
            });
        }

        #[test]
        fn quotient_module_truncation_zeroes_above() {
            Python::initialize();
            Python::attach(|py| {
                // Truncate C2 at degree 0: degree 1 is quotiented to zero.
                let inner = Py::new(py, c2_module(py)).unwrap();
                let q = QuotientModule::new(inner.borrow(py), 0).unwrap();
                q.compute_basis(2);
                assert_eq!(q.dimension(0), 1);
                assert_eq!(q.dimension(1), 0); // above truncation
                assert_eq!(q.max_degree(), Some(0));
                // reduce above the truncation zeroes any vector.
                let v = Py::new(
                    py,
                    crate::fp_py::PyFpVector::from_slice(2, vec![1]).unwrap(),
                )
                .unwrap();
                q.reduce(py, 1, v.bind(py).as_any()).unwrap();
                assert_eq!(v.borrow(py).entry(0).unwrap(), 0);
            });
        }

        #[test]
        fn quotient_module_reduce_projects_out_subspace() {
            Python::initialize();
            Python::attach(|py| {
                let inner = Py::new(py, c2_module(py)).unwrap();
                let mut q = QuotientModule::new(inner.borrow(py), 1).unwrap();
                q.compute_basis(2);
                // Quotient out x1 via the FpVector `quotient` setter.
                let elt = Py::new(
                    py,
                    crate::fp_py::PyFpVector::from_slice(2, vec![1]).unwrap(),
                )
                .unwrap();
                q.quotient(py, 1, elt.bind(py).as_any()).unwrap();
                // Reducing [1] in degree 1 now lands in the complement: 0.
                let v = Py::new(
                    py,
                    crate::fp_py::PyFpVector::from_slice(2, vec![1]).unwrap(),
                )
        ...
        #[test]
        fn quotient_module_out_of_range_and_mutation_after_boxing() {
            Python::initialize();
            Python::attach(|py| {
                let inner = Py::new(py, c2_module(py)).unwrap();
                // truncation below min_degree - 1 is rejected.
                assert!(QuotientModule::new(inner.borrow(py), -5).is_err());

                let mut q = QuotientModule::new(inner.borrow(py), 1).unwrap();
                q.compute_basis(2);
                // Out-of-range degree for the subspace setters raises.
                assert!(q.quotient_basis_elements(5, vec![0]).is_err());
                assert!(q.quotient_all(-1).is_err());
                // Out-of-range basis index raises.
                assert!(q.quotient_basis_elements(0, vec![9]).is_err());
                ...
                .unwrap();
                assert!(q.reduce(py, -1, v.bind(py).as_any()).is_err());

                // After boxing, the Arc is shared, so mutation raises.
                let _boxed = q.into_steenrod_module();
                assert!(q.quotient_basis_elements(0, vec![0]).is_err());
            });
        }

        #[test]
        fn quotient_module_into_steenrod_module_shares_state() {
            Python::initialize();
            Python::attach(|py| {
                let inner = Py::new(py, c2_module(py)).unwrap();
                let q = QuotientModule::new(inner.borrow(py), 1).unwrap();
                q.compute_basis(2);
                let boxed = q.into_steenrod_module();
                assert_eq!(boxed.prime(), 2);
                assert_eq!(boxed.dimension(0), q.dimension(0));
                assert_eq!(boxed.dimension(1), q.dimension(1));
            });
        }
        ...
        #[test]
        fn hom_module_dimensions_and_roundtrip() {
            Python::initialize();
            Python::attach(|py| {
                let algebra = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
                ...
                hom.compute_basis(0);
                // Hom(F<x0>, C2): dim in degree d = target.dim(-d).
                assert_eq!(hom.dimension(-1), 1); // target.dim(1)
                assert_eq!(hom.dimension(0), 1); // target.dim(0)
                assert_eq!(hom.dimension(1), 0); // target.dim(-1)

                // source()/target() round-trip (Arc-shared).
                assert_eq!(hom.source().min_degree(), 0);
                assert_eq!(hom.source().number_of_gens_in_degree(0), 1);
                let t = hom.target();
                assert_eq!(t.dimension(0), 1);
                assert_eq!(t.dimension(1), 1);

                // Scalar action: the field unit fixes a basis element.
                let res = Py::new(
                    py,
                    crate::fp_py::PyFpVector::new(2, hom.dimension(-1)).unwrap(),
                )
                .unwrap();
                hom.act_on_basis(py, res.bind(py).as_any(), 1, 0, 0, -1, 0)
                    .unwrap();
                assert_eq!(res.borrow(py).entry(0).unwrap(), 1);
            });
        }
        ...
        #[test]
        fn hom_module_rejects_mismatch_and_unbounded_target() {
            Python::initialize();
            Python::attach(|py| {
                // Prime mismatch: source over p=2, target over p=3.
                let alg2 = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
                let alg3 = Py::new(py, SteenrodAlgebra::milnor(3, false).unwrap()).unwrap();
                let source = Py::new(py, free_one_gen(py, &alg2)).unwrap();
                let target3 = Py::new(
                    py,
                    ZeroModule::new(alg3.borrow(py), 0).into_steenrod_module(),
                )
                .unwrap();
                assert!(HomModule::new(source.borrow(py), target3.borrow(py)).is_err());

                // Same prime, distinct algebra objects: still rejected.
                let alg2b = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
                let target2b = Py::new(
                    py,
                    ZeroModule::new(alg2b.borrow(py), 0).into_steenrod_module(),
                )
                .unwrap();
                assert!(HomModule::new(source.borrow(py), target2b.borrow(py)).is_err());

                // Unbounded target (a FreeModule) is rejected.
                let unbounded =
                    Py::new(py, free_one_gen(py, &alg2).into_steenrod_module()).unwrap();
                assert!(HomModule::new(source.borrow(py), unbounded.borrow(py)).is_err());
            });
        }

        #[test]
        fn hom_module_out_of_range_no_panic() {
            Python::initialize();
            Python::attach(|py| {
                let algebra = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
                let source = Py::new(py, free_one_gen(py, &algebra)).unwrap();
                let target = Py::new(py, c2_module_over(py, &algebra)).unwrap();
                let hom = HomModule::new(source.borrow(py), target.borrow(py)).unwrap();
                // Below min_degree reads as empty.
                assert_eq!(hom.dimension(-5), 0);
                // Above the populated range reads as empty (no panic).
                assert_eq!(hom.dimension(5), 0);
        ...
        // --- §5.4 FullModuleHomomorphism --------------------------------------

        #[test]
        fn full_mh_zero_invariants_and_apply() {
            Python::initialize();
            Python::attach(|py| {
                let algebra = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
                let m = Py::new(py, c2_module_over(py, &algebra)).unwrap();
                let hom = FullModuleHomomorphism::zero(m.borrow(py), m.borrow(py), 0).unwrap();
                assert_eq!(hom.prime(), 2);
                assert_eq!(hom.degree_shift(), 0);
                assert_eq!(hom.min_degree(), 0);
                // source/target share state and report faithfully.
                assert_eq!(hom.source().dimension(0), 1);
                assert_eq!(hom.target().dimension(1), 1);
                // zero maps every basis element to 0.
                let res = Py::new(py, crate::fp_py::PyFpVector::new(2, 1).unwrap()).unwrap();
                hom.apply_to_basis_element(py, res.bind(py).as_any(), 1, 0, 0)
                    .unwrap();
                assert_eq!(res.borrow(py).entry(0).unwrap(), 0);
                let res1 = Py::new(py, crate::fp_py::PyFpVector::new(2, 1).unwrap()).unwrap();
                hom.apply_to_basis_element(py, res1.bind(py).as_any(), 1, 1, 0)
                    .unwrap();
                assert_eq!(res1.borrow(py).entry(0).unwrap(), 0);
            });
        }

        #[test]
        fn full_mh_identity_is_identity() {
            Python::initialize();
            Python::attach(|py| {
                let algebra = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
                let m = Py::new(py, c2_module_over(py, &algebra)).unwrap();
                let hom = FullModuleHomomorphism::identity(m.borrow(py)).unwrap();
                assert_eq!(hom.degree_shift(), 0);
                assert_eq!(hom.prime(), 2);
                // identity: x0 -> x0 (degree 0) and x1 -> x1 (degree 1).
                let res0 = Py::new(py, crate::fp_py::PyFpVector::new(2, 1).unwrap()).unwrap();
                hom.apply_to_basis_element(py, res0.bind(py).as_any(), 1, 0, 0)
                    .unwrap();
                assert_eq!(res0.borrow(py).entry(0).unwrap(), 1);
        ...
        #[test]
        fn full_mh_from_matrices_known_values_and_aux() {
            Python::initialize();
            Python::attach(|py| {
                let algebra = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
                let m = Py::new(py, c2_module_over(py, &algebra)).unwrap();
                // f: C2 -> C2, degree_shift 0: identity on the bottom cell
                // (degree 0), zero on the top cell (degree 1).
                let m0 = Py::new(
                    py,
                    crate::fp_py::PyMatrix::from_vec(2, vec![vec![1]]).unwrap(),
                )
                .unwrap();
                let m1 = Py::new(
                    py,
                    crate::fp_py::PyMatrix::from_vec(2, vec![vec![0]]).unwrap(),
                )
                .unwrap();
                let hom = FullModuleHomomorphism::from_matrices(
                    m.borrow(py),
                    m.borrow(py),
                    vec![m0.borrow(py), m1.borrow(py)],
                    0,
                    None,
                )
                .unwrap();
                ...
                let r0 = Py::new(py, crate::fp_py::PyFpVector::new(2, 1).unwrap()).unwrap();
                hom.apply_to_basis_element(py, r0.bind(py).as_any(), 1, 0, 0)
                    .unwrap();
                assert_eq!(r0.borrow(py).entry(0).unwrap(), 1);
                let r1 = Py::new(py, crate::fp_py::PyFpVector::new(2, 1).unwrap()).unwrap();
                hom.apply_to_basis_element(py, r1.bind(py).as_any(), 1, 1, 0)
                    .unwrap();
                assert_eq!(r1.borrow(py).entry(0).unwrap(), 0);
                ...
                .unwrap();
                let r2 = Py::new(py, crate::fp_py::PyFpVector::new(2, 1).unwrap()).unwrap();
                hom.apply(py, r2.bind(py).as_any(), 1, 0, input.bind(py).as_any())
                    .unwrap();
                assert_eq!(r2.borrow(py).entry(0).unwrap(), 1);

                // auxiliary data: degree 0 is an iso k->k (image 1, kernel 0);
                // degree 1 is the zero map k->k (image 0, kernel 1).
                hom.compute_auxiliary_data_through_degree(1).unwrap();
                assert_eq!(hom.image(0).unwrap().dimension(), 1);
                assert_eq!(hom.kernel(0).unwrap().dimension(), 0);
                assert_eq!(hom.image(1).unwrap().dimension(), 0);
                assert_eq!(hom.kernel(1).unwrap().dimension(), 1);
                assert!(hom.quasi_inverse(0).is_some());

                // get_partial_matrix on [x0] in degree 0 is the 1x1 matrix [[1]].
                let gm = hom.get_partial_matrix(0, vec![0]).unwrap();
                assert_eq!(gm.rows(), 1);
                assert_eq!(gm.columns(), 1);
                assert_eq!(gm.to_vec(), vec![vec![1]]);
            });
        }

        #[test]
        fn full_mh_guards_raise_not_panic() {
            Python::initialize();
            Python::attach(|py| {
                let algebra = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
                let m = Py::new(py, c2_module_over(py, &algebra)).unwrap();
                ...
                assert!(hom
                    .apply_to_basis_element(py, badp.bind(py).as_any(), 1, 0, 0)
                    .is_err());
                // Uncomputed auxiliary data reads as None (never panics).
                assert!(hom.kernel(7).is_none());
                assert!(hom.image(7).is_none());
                assert!(hom.quasi_inverse(7).is_none());
            });
        ...
        #[test]
        fn full_mh_identity_requires_bounded_module() {
            Python::initialize();
            Python::attach(|py| {
                let algebra = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
                // A FreeModule with a generator is unbounded above; identity
                // must raise rather than letting the upstream `expect` panic.
                let free =
                    Py::new(py, FreeModule::new(algebra.borrow(py), "F".to_string(), 0)).unwrap();
                free.borrow(py).compute_basis(2);
                free.borrow(py).0.add_generators(0, 1, None);
                free.borrow(py).0.compute_basis(2);
                let m = Py::new(py, free.borrow(py).into_steenrod_module()).unwrap();
                assert!(FullModuleHomomorphism::identity(m.borrow(py)).is_err());
            });
        }

        #[test]
        fn full_mh_from_matrices_dimension_mismatch_raises() {
            Python::initialize();
            Python::attach(|py| {
                let algebra = Py::new(py, SteenrodAlgebra::milnor(2, false).unwrap()).unwrap();
                let m = Py::new(py, c2_module_over(py, &algebra)).unwrap();
                ...
                let bad = Py::new(
                    py,
                    crate::fp_py::PyMatrix::from_vec(2, vec![vec![1, 0]]).unwrap(),
                )
                .unwrap();
