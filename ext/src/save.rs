...
pub enum SaveKind {
    ...
    /// The closed-form coboundary `δ_Q` of a field-trick (tensor) resolution `Q• = P• ⊗ M`. This is
    /// module-specific: the header only distinguishes by algebra and bidegree, so the save directory
    /// must be dedicated to a single module `M`.
    TensorDifferential,
}
