    ...
    CC: FreeChainComplex + AugmentedChainComplex + 'static,
{
    ...
    fn min_degree(&self) -> i32 {
        // Bounded below in the filtration direction `s`.
        0
    }

    fn max_computed_degree(&self) -> i32 {
        // Maximum filtration `s` for which the resolution of `M` is defined.
    ...
    fn act_on_basis_multi(
        ...
        // `op ∈ Ext(k, k)` (the ring) acts on `mod ∈ Ext(M, k)` (the module). The product is
        // computed as `mod · op` (equal to `op · mod` up to sign), landing at `op_degree +
        // mod_degree`.
