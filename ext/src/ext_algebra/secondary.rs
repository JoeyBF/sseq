...
//!
//! This mirrors the primary [`ExtAlgebra`](super::ExtAlgebra) / [`ExtModule`](super::ExtModule)
//! split one level up:
//!
//! - [`SecondaryExtAlgebra`] is the **ring** secondary layer over `k`: it wraps an
//!   [`ExtAlgebra`](super::ExtAlgebra) and owns `k`'s secondary resolution and its $E_3$ page.
//! - [`SecondaryExtModule`] is the **module** secondary layer over `M`: it wraps an
//!   [`ExtModule`](super::ExtModule), holds a shared [`Arc`] to the [`SecondaryExtAlgebra`], and
    ...
    pub fn secondary_massey(
        ...
        for c in unit.iter_stem() {
            ...
            let ma = a_lift
                ...
                .get_map(source.s())
                .hom_k(c.t() + b_shift.t());
            let mb = b_lift.underlying().get_map(c.s() + b_shift.s()).hom_k(c.t());
...
mod tests {
    ...
    fn format_bracket(a_name: &str, b_name: &str, r: &SecondaryMasseyResult) -> String {
        ...
        if num_lambda > 0 {
            ...
            s += "λ";
            let basis = BidegreeElement::new(
                r.degree + LAMBDA_BIDEGREE,
                r.multiplicand_lambda.clone(),
            )
            .to_basis_string();
            s += &if num_lambda == 1 {
                basis
        ...
        s += &format!("> = ±[{}] + λ{}", r.ext_part.iter().format(", "), r.lambda_part);
        s
    }
