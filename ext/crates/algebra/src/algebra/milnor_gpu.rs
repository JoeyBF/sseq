...
// Capture/replay lives in `milnor_batch`: it is file IO over `GpuProduct` and owes nothing to
// any GPU framework, so a batch captured by one backend replays against the other and against
// the CPU reference. Re-exported so existing call sites and benches keep working.
pub use crate::algebra::milnor_batch::{capture_batch, load_captured_batch};
