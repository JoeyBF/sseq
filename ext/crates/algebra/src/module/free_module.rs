...
/// lookup against it. Off by default -- turning it on restores the ~127GB the derivation exists to
/// remove, so it is for validation runs at small stems only.
...
pub struct MuFreeModule<const U: bool, A: MuAlgebra<U>> {
    ...
    /// element per degree and measured ~127GB (72% of live heap) at stem 290 -- for data that is
    /// derivable from `generator_to_index` and `gen_deg_idx_to_internal_idx`, both of which are
    /// indexed per GENERATOR and so are smaller by the algebra's dimension. See
    /// [`Self::index_to_op_gen`].
...
impl<const U: bool, A: MuAlgebra<U>> MuFreeModule<U, A> {
    ...
    /// tables: the `generator_to_index` offsets (one `usize` per GENERATOR per degree) plus the
    /// per-degree dimension counters. The per-basis-element table that used to dominate is gone. These scale with Σ dim(t), the same order as a
    /// differential's `outputs`, so they are the other half of the resolution's retained RAM.
    pub fn table_heap_bytes(&self) -> usize {
        let mut bytes = 0usize;
        // The per-basis-element table is gone (derived in `index_to_op_gen`); only the per-degree
        // dimension counters remain from it.
        bytes += (self.dimensions.len() - self.min_degree).max(0) as usize
    ...
    ///
    /// Storing it instead cost one 32-byte record per basis element per degree: ~127GB, 72% of
    /// live heap, at stem 290. The two arrays consulted here are indexed per GENERATOR, of which
    /// there are fewer by a factor of the algebra's dimension.
    ///
