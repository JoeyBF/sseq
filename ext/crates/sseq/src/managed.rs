...
use crate::{
    Adams, Product, Sseq, SseqProfile,
...
pub struct ManagedSseq<P: SseqProfile<2> = Adams> {
    pub p: ValidPrime,
    pub inner: Sseq<2, P>,
...
impl<P: SseqProfile<2>> ManagedSseq<P> {
    ...
    pub fn set_class_name(&mut self, b: Bidegree, idx: usize, name: String) {
        ...
        for prod in self.products.values() {
            ...
            if self.inner.defined(prod_output_b)
                && let Some(flags) = self.stale.get_mut(prod_output_b)
            {
                *flags |= CLASS_FLAG;
            }
        }
