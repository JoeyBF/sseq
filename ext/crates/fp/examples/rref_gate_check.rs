...
//!
//! The gate changed from `min(rows, cols) >= 8192` to `rows * cols >= 2^22 bits`, which admits a
//! large class of wide-and-short matrices that previously always went to CPU M4RI. The existing
//! `cuda_dispatch` tests only cover shapes that were already admitted, and `reduce_shapes` checks
//! rank agreement, which a wrong-but-same-rank reduction would pass.
//!
//! So compare the FULL reduced form, through the real dispatch: `row_reduce` consults the gate and
//! goes to the device, `row_reduce_blas3` is the CPU reference in the same process. (Toggling
...
fn main() {
    // Every one of these was REJECTED by `min(rows,cols) >= 8192` and is admitted by the size gate.
    // The last two are below the new gate and must still be rejected — a gate that admits
    ...
    for &(rows, cols, expect_gpu) in shapes {
        ...
        let r2 = reference.row_reduce_blas3();
