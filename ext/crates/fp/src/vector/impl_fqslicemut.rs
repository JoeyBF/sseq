...
use crate::{
    ...
    limb::Limb,
    vector::{FqSlice, FqSliceMut, FqVectorBase, repr::ReprMut},
};
...
impl<'a, R: ReprMut, F: Field> From<&'a mut FqVectorBase<R, F>> for FqSliceMut<'a, F> {
    fn from(v: &'a mut FqVectorBase<R, F>) -> Self {
        v.as_slice_mut()
    }
}
