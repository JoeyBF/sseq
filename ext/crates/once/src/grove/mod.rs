...
impl<T> Grove<T> {
    ...
    ///
    /// let grove = Grove::<i32>::new();
    /// assert!(grove.try_insert(0, 10).is_ok());
    /// assert!(grove.try_insert(0, 20).is_err()); // Index already occupied
    /// ```
    ...
    /// may be gaps. Also, since this is a lock-free data structure, the return value may not be
...
///
/// This data structure is primarily used as the backing store for the nodes in the backing trie of
/// [`MultiIndexed`](crate::MultiIndexed).
///
/// # Examples
///
/// ```
...
/// assert_eq!(grove.min(), -5);
/// assert_eq!(grove.max(), 5);
/// ```
...
impl<T> TwoEndedGrove<T> {
    ...
    /// assert!(grove.try_insert(-5, 20).is_err()); // Index already occupied
    /// ```
    ...
    /// Returns the minimum index that has a value in the `TwoEndedGrove`.
    ///
    /// # Returns
    ///
    /// The minimum index with a value, or 0 if the `TwoEndedGrove` is empty.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// Returns the maximum index that has a value in the `TwoEndedGrove`.
    ///
    /// # Returns
    ///
    /// The maximum index with a value, or 0 if the `TwoEndedGrove` is empty.
    ///
    /// # Examples
    ///
    /// ```
...
mod tests {
    ...
    fn test_high_contention() {
        high_contention(crate::test_utils::num_threads());
    }
    ...
    fn test_two_ended_grove_basic() {
        let grove = super::TwoEndedGrove::<i32>::new();
    ...
    fn test_two_ended_grove_for_each_mut() {
        let mut grove = super::TwoEndedGrove::<i32>::new();
    ...
    fn test_two_ended_grove_concurrent() {
        ...
        let grove = Arc::new(super::TwoEndedGrove::<i32>::new());
