...
/// A thread-safe, write-once cell that allows a value to be set exactly once.
///
...
/// directly rather than a closure that initializes the value. This allows it to be fully wait-free.
/// This also implies that, whereas `OnceLock` guarantees that the closure is called at most once,
/// `WriteOnce` guarantees that the value is *set* at most once.
///
/// # Thread Safety
///
...
impl<T> WriteOnce<T> {
    ...
    ///
    /// This methods only returns `true` if the value has been set and is ready to be read. In
