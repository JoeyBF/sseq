...
/// A thread-safe, append-only vector optimized for concurrent access.
///
/// `OnceVec` provides a way to build a vector incrementally from multiple threads
/// while maintaining thread safety and memory efficiency. It is particularly useful
/// in scenarios where you need to construct a data structure concurrently and then
/// read from it frequently.
///
/// # Key Features
///
/// - **Thread Safety**: Safe for concurrent reads and writes from multiple threads
/// - **Memory Efficiency**: Uses a paged allocation strategy to minimize memory usage
/// - **Out-of-Order Insertion**: Supports inserting elements at arbitrary positions
/// - **Parallel Extension**: Can be extended in parallel using Rayon (with the `concurrent` feature)
///
/// # Implementation Details
///
/// To ensure thread-safety, `OnceVec` implements three key mechanisms:
///
/// 1. **Paged Allocation**: Instead of a single contiguous allocation that would require
///    reallocation (and invalidate pointers), `OnceVec` uses a [`Grove`] data structure
///    that allocates memory in pages as needed.
///
/// 2. **Mutex-Protected Writes**: A mutex is used to protect write operations, preventing
///    data races during concurrent pushes.
///
/// 3. **Atomic Length Tracking**: The length is stored in an atomic variable that is updated
///    after writing and checked before reading, ensuring that partially written data is never read.
///
/// # Examples
///
/// ```
...
/// # Key Features
///
/// - **Negative Indexing**: Access elements using negative indices
/// - **Thread Safety**: Inherits the thread-safety properties of `OnceVec`
/// - **Memory Efficiency**: Uses the same paged allocation strategy as `OnceVec`
///
/// # Examples
///
/// ```
...
impl<T> OnceBiVec<T> {
    ...
    ///
    /// This is a convenience method for converting from the `bivec` crate's
    /// bidirectional vector implementation.
    ///
    /// # Parameters
    ///
