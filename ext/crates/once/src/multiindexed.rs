...
///
/// `MultiIndexed<K, V>` provides a thread-safe way to store values indexed by multi-dimensional
/// coordinates. It is implemented using a K-dimensional trie structure that efficiently handles
/// sparse data, making it particularly useful for scenarios where the coordinate space is large but
/// only a small subset of coordinates actually contain values.
///
/// The array is created with a fixed number of dimensions `K` and can store values of any type `V`.
/// Each dimension can have both positive and negative indices, allowing for flexible coordinate
/// systems.
///
/// # Thread Safety
///
/// `MultiIndexed` is designed to be thread-safe, allowing concurrent insertions and retrievals from
/// multiple threads. This makes it suitable for parallel algorithms that need to build up a shared
/// data structure.
///
/// # Memory Efficiency
///
...
///
/// - **Insertion**: O(K) time complexity, where K is the number of dimensions
/// - **Retrieval**: O(K) time complexity
/// - **Memory Usage**: O(N) space complexity, where N is the number of inserted elements
///
/// # Warning
///
...
///
/// `MultiIndexed` is implemented as a K-dimensional trie, where each level of the trie corresponds
/// to one dimension of the coordinate space. The trie is built using [`TwoEndedGrove`] instances,
/// which allow for efficient storage of sparse data with both positive and negative indices.
///
/// The trie structure ensures that only coordinates that actually contain values consume memory,
/// making it efficient for sparse data. The implementation also ensures thread-safety by using
...
/// # Thread Safety
///
/// `KdTrie` is designed to be thread-safe, allowing concurrent insertions and retrievals from
/// multiple threads. This is achieved through the use of atomic operations and the thread-safe
/// properties of the underlying [`TwoEndedGrove`] data structure.
///
/// # Memory Efficiency
///
...
impl<V> KdTrie<V> {
    ...
    ///
    /// assert!(trie.try_insert(&[-3, 1], 10).is_ok());
    /// assert!(trie.try_insert(&[-3, 1], 10).is_err()); // Coordinate already occupied
    /// ```
...
/// # Thread Safety
///
/// The `Node` struct is designed to be thread-safe, allowing concurrent insertions and retrievals
/// from multiple threads. This is achieved through the use of atomic operations in the underlying
/// [`TwoEndedGrove`] data structure.
///
/// # Memory Management
///
...
impl<V> Node<V> {
    ...
    unsafe fn ensure_child(&self, idx: i32, to_insert: Self) -> &Self {
        if let Some(child) = self.get_child(idx) {
            ...
            // Either we inserted the node or another thread did. In either case, we can now unwrap
            // the child node.
...
mod tests {
    ...
    fn test_requires_drop() {
        ...
        assert_eq!(
            ...
            (num_threads * inserts_per_thread.pow(1)) as usize
        );
    ...
    mod loom_tests {
        ...
        fn loom_concurrent_insert_get() {
            loom::model(|| {
                ...
                // Stop exploring to avoid the intricacies of the drop mechanism
                loom::stop_exploring();
            });
        }
