...
mod tests {
    ...
    mod proptests {
        ...
        fn operation_strategy<const K: usize>(max: u32) -> impl Strategy<Value = Operation<K>> {
            (0..4u8).prop_flat_map(move |op_type| {
                coords_strategy::<K>(max).prop_flat_map(move |coords| match op_type {
                    0 => proptest::num::i32::ANY
                        .prop_map(move |value| Operation::Insert(coords, value))
                        .boxed(),
                    1 => proptest::strategy::Just(Operation::Get(coords)).boxed(),
                    2 => proptest::strategy::Just(Operation::GetMut(coords)).boxed(),
                    3 => proptest::num::i32::ANY
