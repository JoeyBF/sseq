...
fn main() -> anyhow::Result<()> {
    eprintln!("This script computes Ext(M, N) for unstable modules M and N.");
    let res = ext::utils::query_unstable_module(false)?;
    // let module_spec = query::raw("Module N", ext::utils::parse_module_name);
    let module = ext::utils::query_unstable_module_only("Module N")?;
...
mod hom_cochain_complex {
    ...
    pub struct HomCochainComplex<CC: FreeChainComplex<true>, M: Module<Algebra = CC::Algebra>>
    where
        CC::Algebra: UnstableAlgebra,
    {
        source: Arc<CC>,
        target: Arc<M>,
        modules: OnceVec<Arc<HomModule<M, true>>>,
        differentials: OnceVec<Arc<HomPullback<M, true>>>,
    }

    impl<CC: FreeChainComplex<true>, M: Module<Algebra = CC::Algebra>> HomCochainComplex<CC, M>
    where
        CC::Algebra: UnstableAlgebra,
    {
