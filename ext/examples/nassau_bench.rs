...
//! engine, on the same stem region. Shows the signature shortcut helps the
//! *classical* Adams `E₂` too — independent of the motivic story.
//!
//! ```text
...
fn main() -> anyhow::Result<()> {
    ...
    // Signature engine (stem region {n ≤ max_n, s ≤ max_s}).
    ...
    // Correctness (over the shared stem region).
    let mut mism = 0;
    for s in 0..=max_s {
        for t in 0..=max_t {
            if t - s > max_n {
    ...
    // Master's hand-tuned nassau.rs (p = 2 only), same stem region.
    let master = if p == 2 {
